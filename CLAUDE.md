...
🟡 **수동 천이만 열려 있다** (2026-09-17). SD 스위치(CH7)로 걸린다.
**자동 천이는 계속 막혀 있다** — 미션 이착륙이 `22`/`21` 이라 `84`/`85` 로
바꾸는 순간 미션 시동만으로 천이가 걸린다.

🔴 **에어스피드 영점은 여전히 미판정이다** (정지 중 2.86 m/s). 그래서
`VT_ARSP_TRANS` 를 10 → **14** m/s 로 올려 오차를 흡수해 뒀다 — 이 값은
안전 여유지 올바른 천이 속도가 아니다. **야외 무풍 측정 뒤 되돌려라.**
근거는 [`FC_CHANGELOG.md`](FC_CHANGELOG.md) 2026-09-17 절.
