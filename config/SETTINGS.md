...
> 지금 값은 [`params/px4_params_20260912-184540-preflash.params`](../params/px4_params_20260912-184540-preflash.params),
> 왜 그 값인지는 [FC_CHANGELOG.md](../FC_CHANGELOG.md). 이 문서는 **9/2 당시 구조를
> 사람이 읽기 위한 것**으로만 남긴다 — 값을 인용하지 마라.
>
> 🔴 **2026-09-12 에 펌웨어를 교체했다** (v1.17.0 + dataman 수정, [절차](../docs/firmware/BUILD.md)).
> 그때 사라진 파라미터가 12개 있다 — `MNT_MODE_IN` · `RC_CRSF_PRT_CFG` ·
> `SEP_PORT1/2_CFG` · `TC_A/B/G/M_ENABLE` · `UXRCE_DDS_CFG` ·
> `SYS_CAL_ACCEL/BARO/GYRO`. 전부 "꺼짐" 값이었다.
...
| 미션·펜스 | [`missions/01-square-5pt.plan`](../missions/01-square-5pt.plan) | 2026-09-02 14:24 (2026-09-11 에 `missions/` 로 옮겼다) |
| 펌웨어 | PX4 v1.17.0 커스텀 `d6f12ad1c4f7` | 2026-08-11 빌드 |

> ⚠️ **정본은 FC 다.** 이 스냅샷 이후 기체에서 손으로 바꾼 값은 여기 없다.
> 비행 전에는 QGC 로 다시 받아 대조하라.
>
> 🔴 **스냅샷 이후의 변경은 [`FC_CHANGELOG.md`](../FC_CHANGELOG.md) 에 있다.**
> FC 를 만지기 전에 그것부터 읽고, 만졌으면 거기에 적어라.
...
| `PWM_AUX_TIM0` | ~~400~~ → **현행 100** (2026-09-05 변경, AUX 는 서보뿐) |

**Disarm 값**: MAIN1–2 = 1500 (서보 중립), MAIN3–8 = 1000 (모터 정지).
MIN/MAX 는 전 채널 1000 / 2000.

> ✅ **위 출력 배치표는 2026-09-11 에 로그로 재검증했다.** 이 문서 머리말의
> 「값은 낡았다」 경고는 이 표에는 해당하지 않는다.
> `logs/2026-09-05_09_34_04.ulg` 의 `actuator_outputs` 를 열면 움직이는 것은
> `output[2,3,5,6]`(= MAIN3/4/6/7) 넷뿐이고, `output[0],[1]` 은 1500 고정
> (에일러론), `output[4]` 는 0 (UBEC), `output[7]` 은 1000 고정(크루즈,
> 쿼드 전용)이다 — 표와 정확히 일치한다.
>
> ⚠️ **8/24 이전 로그로 확인하지 마라.** `log_94_2026-8-24` 는 서보가
> `output[4]`·`[7]` 에 있고 크루즈가 `output[1]` 이다 — 서보를 MAIN1–2 로
> 옮기기 전 배치다.
>
> 🔴 **MAVLink 쪽은 번호가 1 부터다.** 웹 실시간(`SERVO_OUTPUT_RAW`)은 같은
> 핀을 `servo3/4/6/7_raw` 로 읽는다. 로그의 `output[2,3,5,6]` 과 숫자가 다른
> 것은 세는 시작이 달라서이고 둘 다 맞다 —
> [web/live/README](../web/live/README.md#모터는-actuator_outputs-에서-읽는다) 에 대조표가 있다.
...
🔴 **USB 조이스틱(HID)은 CH1~8 만 내보낸다.**

✅ **MAVLink 로는 16채널이 전부 온다** (2026-09-09 실측). `RC_CHAN_CNT=16` 이고
`RC_CHANNELS.chancount` 가 16 을 보고한다 — 브리지(USB 직결·ELRS 둘 다)에 제약이 없다.
못 오는 것은 **HID 조이스틱 경로뿐**이다.
...
> 🔴 **이 절의 값 6개는 지금 실기와 다르다** (2026-09-10 전수 대조).
> **비행 판단에 인용하지 마라** — 현행은 오른쪽이다.
>
> | 파라미터 | 이 문서(9/2) | **실기** |
> |---|---|---|
> | `NAV_DLL_ACT` | 0 (동작 없음) | **2 (RTL)** |
> | `GF_ACTION` | 2 (Hold) | **0 (꺼짐)** |
> | `GF_MAX_HOR_DIST` | 150 m | **0** |
> | `GF_MAX_VER_DIST` | 50 m | **0** |
> | `RTL_RETURN_ALT` | 25 m | **20 m** |
> | `MIS_TAKEOFF_ALT` | 5 m | **20 m** |
>
> 🔴 **지오펜스가 꺼져 있다는 것이 특히 중요하다** — 기체는 스스로 멈추지 않는다.
> 현장 판단은 [`docs/emergency/`](../docs/emergency/) 를 봐라.
...
## 5. 미션 (`missions/01-square-5pt.plan`)

> 2026-09-11 에 `config/shade01_20260902-142426.plan` 에서 옮겼다. 미션 정본은
> 이제 [`missions/`](../missions/README.md) 이고, 번호로 관리한다.
...
| 파라미터 | 값 |
|---|---|
| `MIS_TAKEOFF_ALT` | ~~5 m~~ → 🔴 **현행 20 m** (2026-09-05 변경). 순항 5 m 보다 훨씬 높다 |
...
| 🔴 2 | **에어스피드 영점** — 무풍에서 재보정, ±2 이내 | `SENS_DPRES_OFF=-3.0643` |
| ✅ 3 | ~~CH7 천이 매핑 제거~~ — **완료 (2026-09-04)** | `RC_MAP_TRANS_SW` = **0**, FC 저장됨 |
