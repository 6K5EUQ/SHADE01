...
### ⚠️ "SD 는 배제됐다" 로 넘기지 마라 (2026-09-12 추가)

아래 절이 "카드가 아니라 전송 경로" 라고 결론짓는데, **그 결론은 MAVFTP
전송 손상에 한정된다.** FC 내부 SD 접근까지 정상이라는 뜻이 아니다.

2026-09-11 비행에서 미션 3회가 전부 이렇게 실패했다:

```
[dataman_client] timeout after 500 ms!
[navigator] Waypoint could not be read.
```

`dataman` 은 미션을 저장하는 SD 영역이다. 전송이 아니라 **FC 가 자기
카드를 읽다 실패한 것**이고, 같은 날 로그 하나도 전송 중 깨졌다.

🔴 **이 문서를 읽고 "SD 문제는 이미 정리됐다" 고 넘긴 것이 실제로
있었다.** 두 현상은 자리가 다르다 — 증상을 보고 어느 쪽인지 먼저 가려라.

| 증상 | 어디 |
|---|---|
| 받은 파일이 매번 다르다 | MAVFTP **전송** (카드 정상) |
| `dataman timeout` · 미션을 못 읽는다 | **FC 내부 SD 접근** |
