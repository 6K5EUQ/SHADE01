...
- ✅ **실비행 텔레메트리 WiFi 범위 의존 해소** — Pi에 LTE 모뎀 추가(2026-08-31)로 학내 WiFi 범위 밖에서도 인터넷 경유 텔레메트리 링크 유지 가능. 상세: [Pi 문서](../../components/companion/raspberry-pi-5/README.md#-확인-필요)
...
- **별도 구매/확정 완료** (Makeflyeasy 순정품 대신 Holybro 생태계로 구성):
  ...
  - 컴패니언 컴퓨터: [Raspberry Pi 5 "raspb2"](../../components/companion/raspberry-pi-5/README.md) — FC Telem2에 UART(921600) 연결, 자체 제작 `mav_bridge.py`로 MAVLink↔UDP 브리지. ✅ **가동 중**, GCS 2대(`ku-dgs1`/`rim`) 동시 수신 및 배터리 텔레메트리 확인 완료(2026-08-11). ✅ WiFi(`eduroam`) + LTE 모뎀 이중화(2026-08-31)로 WiFi 범위 의존 해소
