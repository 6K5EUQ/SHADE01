...
> 🔴 **2026-09-10: raspb1 이 쇼트로 죽었다. 이 PC 직결 브리지가 지상 정규 경로다.**
> 비행 중 링크는 ELRS 백팩이고, 공중 노드는 없다. 아래 "임시"·"원복" 서술은 raspb1 이
> 살아 있던 때의 것이다.

~~정규 비행 경로는 **raspb1** 이다.~~ PC 직결은 그 자리를 임시로 대신한다
([링크 구성 — 3 경로](../OPERATIONS.md#링크-구성-3-경로)).
...
> 원복한다**~~ — 원복할 raspb1 이 없다. `rim3` 는 기체에 안 실리므로 **비행 중에는 백팩만** 본다.
...
### FC 에 직접 붙는 작업을 할 때 (MAVFTP·파라미터 쓰기)

브리지가 `/dev/ttyACM0` 을 독점하므로 **잠시 멈췄다가 반드시 되살린다.**
상태 파악 → stop → 작업 → start 순서와 함정은
[FC_CHANGELOG 「포트를 쓰는 작업」](../FC_CHANGELOG.md#-포트를-쓰는-작업은-상태-파악--잠시-끄고--우리-것--되살리기) 에 있다.
🔴 `kill` 이 아니라 `systemctl --user stop` 이다 — `Restart=always` 라 되살아난다.
