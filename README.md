...
> ✅ **원래 사유(에어스피드 영점)는 2026-09-11 에 해소됐다.** −5 m/s → **−1.3 m/s**,
> 남은 음수는 PX4 표준 동작임을 참조 기체 실비행 로그로 확인했다
> ([대조](FC_CHANGELOG.md#-2026-09-11--에어스피드-영점-검증-sens_dpres_off--452---18096)).
> **그래도 아직 풀지 않는다** — `ASPD_SCALE_1` 이 **1.0(미보정)** 이고, 비례 오차는
> 고정익으로 날아야만 학습된다. 실속 판단을 좌우하는 것은 영점이 아니라 이쪽이다.
> 푼다면 **높은 고도 · 큰 여유 · 원선회**로 첫 비행을 잡아라 (「다음 비행 전」 3번).
...
2. ✅ **에어스피드 영점 — 해결됐다** (2026-09-11 검증). `SENS_DPRES_OFF` **-4.52 → -1.8096**,
   정지 시 **-1.3 m/s** (30초 ×2회, ±2 기준 충족). 남은 음수는 **정상**이다 — PX4 는 음수를
   설계상 보존하고(`calc_IAS`), 잔차 2.8 Pa 는 MS5525 분해능 급이며, **같은 FC(`PX4_FMU_V6C`)로
   실제 천이한 참조 기체가 호버 중 음수 비율 54.5%(최소 -7.33)로 우리보다 심하다**
   ([대조](FC_CHANGELOG.md#-2026-09-11--에어스피드-영점-검증-sens_dpres_off--452---18096)).
   🟡 **남은 것은 `ASPD_SCALE_1`(현재 1.0, 미보정)이고 이것은 고정익 비행으로만 학습된다** — 아래 3번.
