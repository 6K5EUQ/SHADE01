...
> 🔴 **유지한다** (2026-09-11 저녁, 조종자 지시). 사유가 둘로 늘었다:
>
> 1. **영점이 다시 어긋났다.** 9/11 낮 검증 때 −1.8096 / 정지 −1.3 m/s 로 기준을
>    충족했는데, 저녁 실측은 **−3.0643 / 정지 +2.21 m/s** 다 — 과보정이고 **부호가
>    뒤집혔다.** 양수 오차는 실제보다 빠르게 읽혀 **실속 판단을 늦춘다**
>    (「다음 비행 전」 2번에 표와 근거).
> 2. `ASPD_SCALE_1` 이 여전히 **1.0(미보정)** 이다. 비례 오차는 고정익으로 날아야만
>    학습된다 — 실속 판단을 좌우하는 것은 영점이 아니라 이쪽이다.
>
...
2. 🟡 **에어스피드 영점 — 다시 어긋났다** (2026-09-11 저녁 실측). 조종자가 재보정해
   `SENS_DPRES_OFF` 가 **-1.8096 → -3.0643** 으로 바뀌었는데, **정지 상태 대기속도가
   `-1.3` 에서 `+2.21 m/s` 로 부호가 뒤집혔다** (15초 151표본: 최소 2.03 · 중앙 2.24 ·
   최대 2.38, 흔들림 0.35 m/s).

   | | `SENS_DPRES_OFF` | 정지 대기속도 |
   |---|---|---|
   | 9/10 이전 | -4.52 | **-5.0** (고장) |
   | 9/11 검증 | **-1.8096** | **-1.3** ✅ 기준 충족 |
   | **지금** | **-3.0643** | **+2.21** ⚠️ 과보정 |

   🔴 **과보정이다.** 잔차를 차압으로 되돌리면 2.99 Pa 이고, 이는 지금 오프셋
   3.06 Pa 와 거의 같다 — **뺀 만큼이 그대로 양수로 나오고 있다**는 뜻이다.
   `-1.8096` 일 때가 ±2 기준 안이었으므로 **그 값이 더 낫다.**

   ⚠️ **양수 오차가 음수 오차보다 위험하다.** 음수는 PX4 가 보수적으로 처리하지만,
   양수는 **실제보다 빠르다고 착각**하게 만들어 실속 판단을 늦춘다
   (`FW_AIRSPD_STALL=7`, `FW_AIRSPD_MIN=10`). 정지 중 +2.21 이면 실제 7 m/s 에서
   9 m/s 로 읽어 실속 경보를 놓칠 수 있다.
   🟡 여기에 더해 `ASPD_SCALE_1` 은 여전히 **1.0(미보정)** 이다 — 아래 3번.
...
7. **기압계 −14m 오차** — GPS 대비. `EKF2_HGT_REF=1` 이라 GPS 없는 고도유지에 영향.
