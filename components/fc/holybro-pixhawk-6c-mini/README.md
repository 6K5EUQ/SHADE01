...
| [Holybro PM08-CAN 파워모듈](../../power/holybro-pm08-can/README.md) | **CAN1 (센싱) + Power1 (전원 입력)** — ✅ 분기 결선 완료, QGC 배터리 표시 **정상 작동 확인**(2026-08-11). 아래 참조 |
...
Telem2를 두고 Pi 5와 RC 수신기가 경합하던 문제는 **수신기를 Telem1로 배치**해 해소했다. Pi 링크가 이미 검증·가동 중이었으므로 그쪽을 건드리지 않는 편이 안전했다.
...
PM08에 **동봉된 Y자 분기 케이블**(6핀 → 6핀 + 4핀)로 FC의 두 포트에 나눠 연결했다. 별도 제작 불필요.

| PM08 POWER1 | 전선 색 | → FC 포트 | 신호 |
|---|---|---|---|
| 1, 2 | 빨강 ×2 | **Power1** 핀1·2 | VDD5V_BRICK1 (5.3V) |
| 5, 6 | 검정 ×2 | **Power1** 핀5·6 | GND |
| 3 | 노랑 | **CAN1** 핀2 | CAN1_H |
| 4 | 파랑 | **CAN1** 핀3 | CAN1_L |

- ✅ **작동 확인** — QGC에 배터리 **22.88V / 잔량 48% / 0.17A** 정상 표시.
- **CAN1 핀1(VCC)·Power1 핀3·4(CURRENT1/VOLTAGE1)는 미연결** — 모두 의도된 정상 상태. VCC는 연결 시 5V 소스 충돌 위험, CURRENT1/VOLTAGE1은 아날로그 파워모듈용이라 CAN 센싱에는 불필요.
- 🔶 **CAN1 핀4(GND) 미결선** — FC 내부에서 Power1 GND와 공통이라 현재 정상 동작하나, 모터 구동 시 노이즈 환경에서 재확인 필요.

**남은 제약: FC 전원 이중화 불가** — 6C Mini에는 Power2가 없어 PM08의 5.3V 2계통 중 1개만 사용 가능. PM08의 POWER2는 유휴로 남는다.

상세: [PM08-CAN 문서의 실제 결선](../../power/holybro-pm08-can/README.md#-실제-결선-2026-08-11--작동-확인-완료)

## 커스텀 펌웨어 (PX4 v1.17.0 + `crsf_rc`)

✅ **빌드·플래시 완료 (2026-08-11).** [RP4TD-M 수신기](../../receivers/radiomaster-rp4td-m/README.md)의 CRSF 프로토콜을 쓰려면 기본 배포 펌웨어로는 안 되고, `crsf_rc` 드라이버를 포함해 직접 빌드해야 한다.

| 항목 | 값 |
|---|---|
| PX4 버전 | **v1.17.0** (안정 릴리스) |
| 빌드 타깃 | `px4_fmu-v6c_default` |
| 산출물 | `px4_fmu-v6c_default.px4` (1.8MB) |
| 추가 드라이버 | `crsf_rc` (기본 `rc_input` 대신) |
| **플래시 사용률** | **98.30%** ⚠️ |

**검증**: 플래시 후 QGC Parameters에 `RC_CRSF_PRT_CFG`가 나타나는 것으로 드라이버 정상 탑재를 확인했다. 펌웨어 버전은 Vehicle Setup → Summary에서 `PX4 v1.17.0`으로 표시된다.

### ⚠️ 플래시 용량 98.30%의 부작용

용량이 빠듯해 PX4가 v6c 보드 기본 설정에서 **일부 모듈을 제외**한다. 그 결과:

- QGC가 **"파라미터 누락" 경고**를 띄우는 경우가 있다. QGC는 표준 릴리스 기준으로 파라미터 목록을 검사하기 때문이며, 커스텀 빌드에서는 흔한 현상이다.
- 경고 자체보다 **실제 기능 오작동 여부**로 판단해야 한다. 파라미터가 목록에 없다고 곧바로 문제인 것은 아니다.
- 추가 모듈을 더 넣으려면 다른 모듈을 빼야 한다.

### 🔶 확인 필요

- **펌웨어 교체 전 버전 미기록** — 교체 이전 FC에 어떤 버전이 올라가 있었는지 확인하지 못한 채 v1.17.0을 플래시했다. 이전이 더 최신이었다면 다운그레이드이며, 일부 파라미터명이 달라 복원 시 누락이 생길 수 있다.
- **MAVLink over ELRS 대안 미검토 완료** — ELRS가 MAVLink를 CRSF 형식으로 변환해 EdgeTX 화면 표시와 QGC 연결을 동시에 해결하는 방식이 있다. 이 경로를 쓰면 `RC_CRSF_PRT_CFG` 대신 `MAV_2_CONFIG=TELEM1` + `SER_TEL1_BAUD=460800`을 쓰며, **CRSF 커스텀 펌웨어가 불필요**했을 수 있다. 다만 빌드해 둔 v1.17.0 펌웨어에 드라이버가 포함된 것은 손해가 아니고, 이 대안이 여의치 않을 때 그대로 쓸 수 있다. ELRS 펌웨어 **3.5.0 이상** 필요.

## 채널 배정안 (PX4 기준)

> ✅ **SHADE 기체는 PX4 확정** (2026-08-04)

소요 10채널(ESC 5 + 서보 5) < 보유 14채널.

### ✅ 실기 배치 (2026-09-01 FC 실측 — 확정)

아래는 **FC 파라미터를 직접 읽어 확인한 현재 배치**다. 이전 판의 "관례 기반 제안" 표는
실기와 달랐으므로 폐기했다.

| 헤더 | 채널 | `FUNC` | 기능 | PWM | 해제 시 |
|---|---|---|---|---|---|
| **MAIN** | **1** | 202 | **우 에일러론** (Servo2) | **100Hz** | 1500 중립 |
| **MAIN** | **2** | 201 | **좌 에일러론** (Servo1) | **100Hz** | 1500 중립 |
| MAIN | 3 | 102 | VTOL **우후** (Motor2) | 400Hz | 1000 |
| MAIN | 4 | 103 | VTOL **우전** (Motor3) | 400Hz | 1000 |
| **MAIN** | **5** | 0 | **UBEC 5.3V 입력** (신호 미사용) | 400Hz | 1000 |
| MAIN | 6 | 104 | VTOL **좌후** (Motor4) | 400Hz | 1000 |
| MAIN | 7 | 105 | VTOL **좌전** (Motor5) | 400Hz | 1000 |
| **MAIN** | **8** | 101 | **크루즈 모터** (Motor1) | 400Hz | 1000 |
| AUX | 1 | 204 | **엘리베이터** (Servo4, CS3) | — | 1500 |
| AUX | 2 | 205 | **러더** (Servo5, CS4) | — | 1500 |
| AUX | 3 | 203 | **엘리베이터** (Servo3, CS2) | — | 1500 |
| AUX | 4~8 | 0 | 미사용 | — | — |

전 채널 `MIN/MAX` = 1000/2000, `FAIL` = −1(비활성).

### 🔴 PWM 타이머 그룹 — 서보/모터를 섞지 마라

**PWM 주파수는 채널별이 아니라 타이머 그룹별로만 설정된다.** 6C Mini 의 그룹은
QGC Actuators 화면에서 확인한 실측 기준으로 다음과 같다:

| 그룹 | 채널 | 현재 |
|---|---|---|
| `PWM_MAIN_TIM0` | **MAIN1–2** | **100Hz** (서보 전용) |
| `PWM_MAIN_TIM1` | MAIN3–4 | 400Hz (모터) |
| `PWM_MAIN_TIM2` | MAIN5–8 | 400Hz (모터) |

⚠️ **서보에 400Hz 는 위험하다.** 업계 자료는 400Hz 가 대부분의 서보를 손상시킨다고
경고하며 안전 범위를 40~200Hz 로 본다. [MFE S3054](../../servos/mfe-s3054/README.md) 는
제조사가 갱신 주파수를 아예 기재하지 않았다.

2026-08-31 까지는 **서보가 모터와 같은 그룹에 섞여 400Hz 로 구동됐다** (8/22 부터 계속).
비행에 즉시 문제는 없었으나 수명을 갉아먹는 조건이었다. 2026-09-01 배선을 옮겨
**서보 2개를 MAIN1–2 한 그룹에 모아** 그 그룹만 100Hz 로 낮췄다 — 모터는 400Hz 를 유지한다.

> ⚠️ **`PWM_MAIN_TIM*` 변경은 재부팅해야 IO 보드에 반영된다.** 파라미터만 바꾸고
> `px4io status` 의 `pwm_rate_hz` 를 보면 옛 값 그대로다. 실측으로 확인한 사항이다.

### 배선을 옮길 때 함께 옮겨야 하는 것

포트를 바꾸면 `PWM_*_FUNC` 만으로는 부족하다. **기능을 따라가는 값들이 있다:**

| 파라미터 | 성격 |
|---|---|
| `PWM_MAIN_FUNC<N>` | 그 핀이 무엇인지 |
| `PWM_MAIN_DIS<N>` | 시동 해제 시 출력 — **서보 1500(중립) / 모터 1000(정지)** |
| `PWM_MAIN_REV` | 반전 **비트마스크** (bit0=MAIN1). 반전 서보를 옮기면 비트도 옮겨야 한다 |
| `PWM_MAIN_MIN/MAX<N>` | 가동 범위. 본 기체는 전 채널 1000/2000 |

`DIS` 를 안 옮기면 시동 해제 상태에서 **서보가 한쪽 끝까지 꺾이거나** ESC 가 중립 신호를
받는다. `REV` 를 안 옮기면 조종면이 **반대로** 움직인다.

믹서(`CA_SV_CS*`, `CA_ROTOR*`)는 Servo/Motor **번호**로 동작하므로 물리 포트 변경과 무관하다.
...
- ~~펌웨어 PX4/ArduPilot 미확정~~ → **해소(2026-08-04): PX4 확정.** → **빌드·플래시까지 완료(2026-08-11)**: PX4 v1.17.0 + `crsf_rc`. [커스텀 펌웨어](#커스텀-펌웨어-px4-v1170--crsf_rc) 참조
- ~~Telem2 포트 충돌 (Pi 5 ↔ RC 수신기)~~ → **해소(2026-08-19)**: 수신기를 **Telem1(UART7)**로 배치, Pi 5는 Telem2 유지. [UART 포트 배정](#-uart-포트-배정--충돌-해소-2026-08-19) 참조
- ~~PM08 6핀 커넥터의 핀 배열 미확보 / CAN 센싱만으로 동작하는지 검증~~ → **해소(2026-08-11)**: 분기 결선 완료, CURRENT1/VOLTAGE1 미결선 상태로 **QGC 배터리 표시 정상 확인**. [PM08-CAN 연결](#pm08-can-연결---결선-완료-2026-08-11) 참조
