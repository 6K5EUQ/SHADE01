...
## 🔴 링크 대역폭 포화 — "Sensor lost" 의 원인 (2026-09-04 규명)
...
### 대역폭 — ELRS 설정으로 확정 (2026-09-04)

> 🔴 **이 절의 250Hz 는 낡았다 (2026-09-07 정정).** 조종기 현재 설정은
> **333Hz Full / Telem Ratio 1:2** 이고, 그 상한은 **1470 B/s** 다.
> 아래 615 B/s 계산과 "96% 포화" 결론은 **250Hz 시절의 것**이므로 그대로 인용하지 마라.
>
> 이 착오가 실제로 오판을 낳았다 — 2026-09-05 에 `MAV_0_RATE` 를 490→300 으로
> 내렸다가(포화로 오진) 4분 만에 되돌렸다. 지금 기준으로 다시 계산하면:
>
> | | 값 | 상한 대비 |
> |---|---|---|
> | ELRS 상한 (333Hz Full 1:2) | **1470 B/s** | — |
> | `MAV_0_RATE` 송신 상한 | **490 B/s** | 33% |
> | 실제 도착 (tlog 실측) | **290 B/s** | 20% |
>
> **병목은 대역폭이 아니라 `MAV_0_RATE=490` 이다.** 1180 B/s 가 남아돈다.
...
**적용 후에도 지상에서 `txbuf` 는 여전히 0 이다.** UART 버스트 특성상 평균 대역폭을
낮추는 것만으로는 순간 포화를 못 막기 때문이다. 실효 검증은 **비행 로그의
`rate_multiplier`** 로 해야 한다.
