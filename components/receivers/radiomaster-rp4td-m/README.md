...
| | 값 | 근거 |
|---|---|---|
| PX4 가 믿는 예산 | **23040 B/s** | `MAV_0_RATE=0` → `SER_TEL1_BAUD/20` |
| 실제 ELRS 무선 처리량 | **583~638 B/s** | 9/2 로그 5개 `tx_rate_avg` |
| 차이 | **약 38배** | |

`460800` 은 **FC↔RX 유선 UART** 속도다. 그 구간은 실제로 그 속도가 나온다.
문제는 PX4 가 그 숫자를 **무선** 대역폭으로 착각하는 것이다.
...
| OTA 1패킷 페이로드 | **최대 62 바이트** (`SerialMavlink.cpp:159`) |
| `txbuf` 의 정체 | 그 버퍼의 **잔량 %** (`SerialMavlink.cpp:90`) |

460800 으로 밀어넣고 62바이트씩 무선으로 뺀다. 유입이 배출을 압도해 버퍼가 즉시 찬다.
...
**감산 20% 대 회복 2.5% — 비대칭이다.** 한 번 떨어지면 잘 안 올라온다.
하한은 `0.05` (`mavlink_main.cpp:1352`).
...
**`MAV_0_RATE = 700`** (2026-09-04 적용). 실측 평균 600 + 헤드룸 15%.
PX4 가 처음부터 예산 안에서 스케줄링하므로 버퍼가 안 넘치고 스트림이 균등 배분된다.
...
```bash
./qgc log 1     # rate_multiplier 가 1.0 근처면 성공
```

`txbuf` 가 50 이상을 유지하고 `rate_multiplier` 가 0.5 위로 올라오면 해소된 것이다.
