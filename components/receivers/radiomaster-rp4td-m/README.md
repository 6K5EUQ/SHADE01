...
## ⛔ 이 절은 오진이었다 — "Sensor lost" 의 원인은 대역폭이 아니다 (2026-09-09 정정)

> 🔴 **아래 「대역폭 포화」 진단은 틀렸다. 인용하지 마라.**
>
> 진짜 원인은 **`MAV_0_FORWARD=1`** 이다. TELEM1 이 내보내던 2868 B/s 를 분해하니
> 스트림은 **132 B/s(5%)** 뿐이고 나머지 **2736 B/s(95%)가 USB→TELEM1 포워딩**이었다.
> `rim3` USB 브리지로 들어온 트래픽이 통째로 조종기 링크로 밀려나가고 있었다 —
> **지상에서 QGC·브리지를 붙일수록 조종기가 죽는 구조**였다.
>
> | | 오진 (2026-09-04) | 실측 (2026-09-09) |
> |---|---|---|
> | 링크 설정 | 250Hz / 1:2 | **333Hz Full / 1:2 / 13211bps** |
> | OTA 상한 | 615 B/s | **1651 B/s** |
> | 병목 | "대역폭 96% 포화" | **`MAV_0_FORWARD` 포워딩 95%** |
>
> 이 착오가 실제로 오판을 낳았다 — 2026-09-05 에 `MAV_0_RATE` 를 490→300 으로
> 내렸다가(포화로 오진) 4분 만에 되돌렸고, 9/7 에 990 으로 올렸어도 증상은 그대로였다.
> 병목이 거기가 아니었기 때문이다.
>
> **해결과 실측**: [FC_CHANGELOG](../../../FC_CHANGELOG.md#-2026-09-09--sensor-lost-원인-규명과-해결-mav_0_forward--extrastxt--mav_0_rate).
> 배터리 최대공백 **40.02s → 0.81s**, `BAD_DATA` **253 → 0**.
>
> ---
>
> 아래 원문은 **당시 기록으로만** 남긴다. 계산 방법(슬롯당 5바이트, `txbuf` 의 의미,
> PX4 의 `rate_multiplier` 비대칭 감산)은 여전히 옳고, **틀린 것은 상한값과 결론**이다.

**증상**: 조종기에서 **"Sensor lost"** 음성이 반복된다. 실내·야외 무관하게 난다.
링크는 안 끊긴다 (RSSI 정상, `rxerrors=0`).

~~**원인**: ELRS OTA 대역폭이 PX4 의 송신 예산보다 **38배 작다.**~~

### 대역폭 — ELRS 설정으로 확정 (2026-09-04) ⛔ 값이 틀렸다

조종기 ELRS 화면 실측(2026-09-04 당시): **Packet Rate 250Hz / Telem Ratio 1:2 / 4921bps**

이 값에서 나오는 하향(FC→조종기) 텔레메트리 상한:

| | 값 | 근거 |
|---|---|---|
| 텔레메트리 슬롯 | 250 ÷ 2 = **125 회/초** | Telem Ratio |
| 슬롯당 페이로드 | **5 바이트** | `ELRS4_DATA_DL_BYTES_PER_CALL` |
| **OTA 상한** | **625 B/s** (= 5000bps) | 계산 |
| ELRS 자체 표기 | **615 B/s** (= 4921bps) | 동기 패킷 오버헤드 차감 |

계산과 ELRS 표기가 **1.6% 이내로 일치**한다. 상한은 **615 B/s** 로 본다.
...
|---|---|
| ELRS OTA 상한 | **615 B/s** |
| PX4 가 밀어넣던 양 | **593 B/s** (9/2 로그 `tx_rate_avg`) |
| 여유 | **3.6%** |

**여유가 없다.** PX4 는 이미 `rate_multiplier` 0.147 까지 조여진 상태에서도 상한의
**96%** 를 쓰고 있었다. 조금만 튀어도 RX 의 1024바이트 버퍼가 넘친다.

> ⚠️ **초기 분석의 오류 정정.** 처음에 "PX4 예산 23040 B/s vs 실제 600 B/s, 38배 초과"
> 라고 적었으나 **틀렸다.** `tx_rate_avg` 593 B/s 는 ELRS 의 처리량이 아니라 PX4 가
> 내보낸 양이다. 실제 상한(615)과 비교하면 38배가 아니라 **96% 포화**다.
> `MAV_0_RATE=0` 일 때 PX4 의 *예산*이 23040 인 것은 맞지만, 스로틀 로직이 이미
> 실효 송신량을 상한 근처로 눌러 두고 있었다.

### ELRS 쪽 구조 (소스 확인)

| | |
|---|---|
| RX 입력 버퍼 | **1024 바이트** (`SerialMavlink.h:6`) |
| OTA 패킷 | **8 바이트**, 그중 텔레메트리 **5 바이트** (`OTA.h:13`, `telemetry_protocol.h:4`) |
| `txbuf` 의 정체 | 그 버퍼의 **잔량 %** (`SerialMavlink.cpp:90`) |

FC 는 UART 460800(≈46000 B/s)으로 **버스트**로 밀어넣고, OTA 는 8ms 마다 5바이트씩만
뺀다. 한 슬롯 주기(8ms)에 UART 로는 약 **369 바이트**가 들어올 수 있다 — 버퍼가
순간적으로 차는 것을 평균 대역폭만으로는 못 막는다.

### PX4 의 반응 — 스스로 5% 까지 조인다

ELRS 가 `RADIO_STATUS.txbuf` 로 버퍼 잔량을 알리면 PX4 가 송신률을 깎는다
(`mavlink_main.cpp:1365`):

| txbuf | PX4 동작 |
|---|---|
| < 25 | `_radio_status_mult *= 0.80` — **20% 감산** |
| < 35 | `*= 0.975` |
| > 50 | `*= 1.025` — 회복은 **2.5% 씩만** |

**감산 20% 대 회복 2.5% — 비대칭이다.** 하한은 `0.05` (`mavlink_main.cpp:1352`).

### 9/2 야외 비행 실측

| 로그 | 비행 | `rate_multiplier` 평균 | 0.5 미만 시간 | `txbuf` 평균 | `txbuf==0` |
|---|---|---|---|---|---|
| log_177 | 102초 | 0.154 | **100%** | 49.1 | 30 |
| log_181 | 92초 | 0.156 | **100%** | 51.5 | 89 |
| log_182 | 180초 | 0.150 | **100%** | 51.8 | 83 |
| **log_184** | 198초 | **0.050** ← 바닥 고정 | **100%** | **8.7** | **10386** |
| log_187 | 306초 | 0.147 | **100%** | 48.2 | 545 |

**전 비행시간 100% 가 스로틀 상태.** log_184 는 회복 구간이 한 번도 없었다.

### 왜 "Sensor lost" 로 들리는가

```
대역폭 포화 → rate_multiplier 0.05 → 저빈도 스트림부터 굶는다
   → BATTERY_STATUS · GPS_RAW_INT 등이 수 초~수십 초 끊김
   → EdgeTX 가 해당 센서 timeout → sensorko.wav "Sensor lost"
   → 잠깐 돌아오면 recover, 또 굶으면 재발 → 반복
```

`ATTITUDE`(고빈도)는 살아남고 저빈도 센서가 먼저 죽는다. 그래서 자세는 멀쩡한데
특정 칸만 `---` 로 깜빡인다. **링크 상실(`rxko`)이 아니라 센서 상실(`sensorko`)** 인
이유가 이것이다.

### 조치 ⛔ 이 값은 옛것이다 — 현재 `MAV_0_RATE = 1400`

> **현재 값은 `1400`** 이다 (2026-09-09). 아래 490 은 2026-09-04 당시의 것이고,
> 그 사이 490 → 300 → 490 → 990 → **1400** 으로 옮겨 다녔다. 490·990 어느 쪽도
> 증상을 못 고쳤다 — 병목이 `MAV_0_RATE` 가 아니라 `MAV_0_FORWARD` 였기 때문이다.
> 1400 은 실측 OTA 상한 1651 B/s 의 **85%** 이고, 실제 송신은 900~1080 B/s 다.

~~**`MAV_0_RATE = 490`** (2026-09-04 적용, FC 재부팅 완료).
ELRS 상한 615 B/s 의 **80%** 다. 나머지 20% 를 버스트 여유로 남긴다.~~

> 🔴 **FC 재부팅해야 적용된다.** `_datarate` 는 인스턴스 기동 시 한 번만 읽는다
> (`mavlink_main.cpp:2203`). 값만 쓰고 재부팅을 안 하면 아무것도 안 바뀐다.
>
> ⚠️ `MAV_0_RATE` 는 **TELEM1(`MAV_0_CONFIG=101`) 인스턴스에만** 적용된다.
> USB 링크는 별도 인스턴스라 영향이 없다 — **USB 로 보면서 검증할 수 없다.**

~~**적용 후에도 지상에서 `txbuf` 는 여전히 0 이다.**~~ ⛔ 이것도 옛 상태다.
`MAV_0_FORWARD=0` 이후 지상 `txbuf` 는 **100**(여유 100%), `rate_multiplier` 는
**1.000**(무감산)이다. `txbuf` 가 0 으로 눌려 있던 것이 포워딩 트래픽의 증상이었다.

**검증법** — 다음 비행 로그로:

```bash
./qgc log 1     # rate_multiplier 가 0.5 위로 올라왔으면 개선
```

9/2 기준값은 평균 0.147 / 100% 가 0.5 미만이었다. 이보다 올라가면 효과가 있는 것이다.

### 더 확실한 해결 — Telem Ratio 를 올린다

`MAV_0_RATE` 는 **PX4 가 덜 보내게** 하는 것이라 텔레메트리 갱신이 그만큼 느려진다.
**대역폭 자체를 늘리는 쪽이 근본적이다.**

현재 **1:2 가 이미 최대치**다 (1:2 → 1:128 순으로 텔레메트리가 줄어든다).
따라서 남은 카드는 **Packet Rate 를 올리는 것**:

| Packet Rate | Telem 1:2 상한 | 비고 |
|---|---|---|
| 250Hz (현재) | **625 B/s** | |
| 333Hz | 832 B/s | +33% |
| 500Hz | **1250 B/s** | **+100%** — 여유 충분 |

⚠️ **Packet Rate 를 올리면 감도(수신 거리)가 줄어든다.** 500Hz 는 250Hz 보다
링크 마진이 낮다. 근거리 비행 위주면 500Hz 가 유리하고, 장거리면 250Hz 를 유지하고
`MAV_0_RATE` 로 맞추는 편이 안전하다.

**판단**: 현재 운용(펜스 150m)은 근거리다. **500Hz + 1:2** 로 올리면
대역폭이 2배가 되어 `MAV_0_RATE` 를 1000 근처까지 풀 수 있고,
"Sensor lost" 와 백팩 QGC 로딩이 **동시에** 해소된다.

**추가 여지** — 그래도 부족하면 `MAV_0_MODE` 를 `0`(Normal, 58스트림) 에서
`2`(Onboard) 나 `7`(Minimal) 로 내린다. ⚠️ 먼저 `shade.lua` 가 쓰는 센서
(`Curr` `Bat%` `GSpd` `GAlt` `Temp` `Sats`) 가 살아남는지 확인할 것.
