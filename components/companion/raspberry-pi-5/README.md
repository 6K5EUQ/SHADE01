...
[Pixhawk 6C Mini] ──Telem2 UART 921600──► [Raspberry Pi 5 "raspb2"]
                                        ...
                                        (UDP :14550 브리지)
                                                  │
                                    WiFi(eduroam) 또는 LTE → 인터넷 → Tailscale
                                                  │
                                    ┌─────────────┴─────────────┐
                                    ▼                           ▼
...
- Pi의 인터넷 연결은 **WiFi(`eduroam` 클라이언트 모드, wlan0: 10.200.114.166/17) + LTE 모뎀 이중화** (2026-08-31 LTE 추가). WiFi는 AP 모드 아님 — 기체가 자체 핫스팟을 띄우는 구조가 아니다. LTE 모뎀 기종·연결 방식은 별도 기록 필요.
...
- ✅ **WiFi 범위 의존 해소 — LTE 모뎀 추가 (2026-08-31)** — 기존에는 `eduroam` WiFi 단독이라 학내 범위를 벗어나면 링크가 끊겼으나, LTE 모뎀을 추가해 WiFi 범위 밖에서도 인터넷 경유(Tailscale) 텔레메트리가 유지되는 구조로 전환. 실비행 검증(신호 강도, 레이턴시, 배터리 소모 등)은 별도 확인 필요.
