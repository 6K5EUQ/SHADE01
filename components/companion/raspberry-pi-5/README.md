...
> ### 🔴 raspb1 은 쇼트로 죽었다 (2026-09-10 확정) — 이 문서는 기록이다
>
> 9/4 부터 offline 이던 것이 하드웨어 사망으로 확정됐다. FC USB 는 `rim3` PC 에 있고
> 비행 링크는 ELRS 백팩이다 — [README 링크 구성](../../../OPERATIONS.md#링크-구성-3-경로).
> **이 Pi 는 BEWE DGS-3 지상 기지도 겸했으므로 DGS-3·드론 데이터링크·공중 노드가 전부
> 불가**다. 기체 탑재 컴패니언이 다시 필요하면 **새 보드**에 아래 구성(USB 직결 브리지,
> `mavlink-bridge.service`, WiFi+LTE)을 재현한다. 아래 "현재" 는 8/31~9/3 시점이다.
>
> ### 🔵 링크 구성 — **FC USB ↔ raspb1** (2026-08-31 실측 · 9/4 부터 휴면)
>
...
> **raspb1 은 기체에 실리는 유일한 컴패니언이다.** raspb2 는 고장으로 제거됐다.
> FC ↔ Pi 는 USB, Pi ↔ GCS 는 Tailscale UDP 14550.
>
> 지상국 링크 자체는 셋이다 — raspb1 브리지 / PC USB 직결 / ELRS 백팩
> ([링크 구성 — 3 경로](../../../OPERATIONS.md#링크-구성-3-경로)). 다만 **비행 중 쓸 수 있는
> 컴패니언 경로는 raspb1 뿐**이다: PC 직결은 기체에 안 실리고, 백팩은 조종기 AP 라
> 사거리가 다르다.
>
> 🟡 **2026-09-04 — raspb1 잠정 중단.** FC USB 를 `rim3` 로 옮겼다. 비행 전 원복 필수.
