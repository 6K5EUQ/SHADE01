...
**① 전원 입력 ↔ [PDB 300A](../../power/holybro-pdb-300a-side-entry/README.md#-pdb--esc-커넥터-불일치--무개조-연결-불가-2026-08-08-확인) — 🔴 그대로 연결 불가**

본 ESC의 전원 입력은 **OT2.5-4 링터미널**인데 PDB의 ESC 출력단은 **XT90**이다. 암수 문제가 아니라 커넥터 종류가 달라, XT90 납땜 교체 또는 어댑터 케이블 제작이 필요하다. PDB의 스크류터미널은 BAT-IN 전용이라 대안이 되지 않는다. 상세는 PDB 문서 참조.
...
관행상 모터=수/ESC=암이면 맞아떨어지나 **추정이며 확인된 사실이 아니다.** 특히 이 조합은 [모터 모델 불일치 의혹](#️-모터-호환-경고-제조사-원문)이 미해소 상태이므로 커넥터까지 추정으로 넘기지 말 것. 크루즈용 [ESC 6S 100A](../mfe-esc-6s-100a/README.md)는 **4.0mm로 직경이 다르므로** 별개로 확인해야 한다.
