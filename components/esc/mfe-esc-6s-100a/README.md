...
본 ESC의 전원 입력은 원래 **OT2.5-4 링터미널**이고 PDB의 ESC 출력단은 **XT90**이라 그대로는 연결할 수 없었다. 링터미널을 잘라내고 커넥터를 **직접 납땜**하여 해소했다.

본 ESC는 **100A 정격으로 XT90의 연속 45A를 크게 초과**하나, 첫 비행에서 이상 없음이 확인되었다(2026-08-31). 장시간·고부하 임무 시 커넥터 발열 경과 관찰은 계속 권장한다 ([PDB 연결 구조](../../power/holybro-pdb-300a-side-entry/README.md#연결-구조-striver-기체-기준) 참조).
