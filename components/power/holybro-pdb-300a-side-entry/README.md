...
### ✅ PDB ↔ ESC 커넥터 불일치 — 납땜 개조로 해소 (2026-08-31 완료)
...
**대응 (완료)** — ESC 측 링터미널을 잘라내고 커넥터를 **직접 납땜**하여 해소. 어댑터 케이블을 거치지 않으므로 접점 추가로 인한 발열 지점도 없다.

> ℹ️ 아래 전류 정격 항목(크루즈 ESC 100A vs XT90 45A)은 **첫 비행에서 이상 없음이 확인**되었다(2026-08-31). 다만 커넥터 정격 자체를 바꾼 것은 아니므로, 장시간·고부하 임무 시 발열 경과 관찰은 계속 권장한다.
