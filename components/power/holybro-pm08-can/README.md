...
> 출처: [제품 페이지 저장본 PDF](images/screencapture-holybro-products-dronecan-pm08-power-module-14s-200a-2026-08-08-10_52_19.pdf) p.2 "Pinout" 섹션 — 커넥터 실물 사진 아래 핀 배열 도해에 `5V / 5V / CAN-H / CAN-L / GND / GND` 표기.
...
### ✅ 실제 결선 (2026-08-11 — 작동 확인 완료)

**동봉된 Y자 분기 케이블**(아래 [동봉 케이블](#동봉-케이블-구성) 참조)로 PM08 POWER1 6핀을 FC의 두 포트에 나눠 연결했다. 6가닥의 색상과 행선지:

| PM08 POWER1 핀 | 전선 색 | → Pixhawk 6C Mini | 신호 |
|---|---|---|---|
| 1 | **빨강** | Power1 핀1 | VDD5V_BRICK1 |
| 2 | **빨강** | Power1 핀2 | VDD5V_BRICK1 |
| 5 | **검정** | Power1 핀5 | GND |
| 6 | **검정** | Power1 핀6 | GND |
| 3 | **노랑** | **CAN1 핀2** | CAN1_H |
| 4 | **파랑** | **CAN1 핀3** | CAN1_L |
...
PM08 POWER1 (6핀 1개)
   │
   ├─ 빨강 ×2, 검정 ×2 (4줄) ──► FC Power1  (전원)
   └─ 노랑 ×1, 파랑 ×1 (2줄) ──► FC CAN1    (센싱)
```
...
| 핀 | 상태 | 이유 |
|---|---|---|
| FC CAN1 핀1 (VCC) | 미연결 | FC가 CAN 주변장치에 주는 5V 출력. PM08은 자체 전원이라 연결 시 **5V 소스 충돌** |
| FC Power1 핀3·4 (CURRENT1/VOLTAGE1) | 미연결 | 아날로그 파워모듈용 센싱 핀. PM08은 CAN으로 보내므로 불필요 |
| PM08 POWER2 | 미사용 | 6C Mini에 Power2 포트 없음 ([전원 이중화 불가](#2-fc-전원-이중화-불가)) |

**작동 검증**: QGC에서 **22.88V / 셀당 3.81V / 잔량 48% / 0.17A** 정상 표시. FC 전원 인가와 CAN 센싱이 모두 성립함이 확인됐다. (경로: PM08 → FC → [Raspberry Pi 5](../../companion/raspberry-pi-5/README.md) → QGC)

> ⚠️ **CAN1 핀4(GND)는 미결선 상태** — 위 결선표 권장안은 PM08 6번 핀에서 GND를 분기해 CAN1 핀4에도 넣는 것이나, 실제로는 CAN에 노랑·파랑 2줄만 갔다. **현재 정상 동작한다** — Power1의 GND와 CAN1의 GND가 FC 기판 내부에서 공통이라 기준전위가 Power1 쪽 검정선으로 잡히기 때문이다.
>
> 다만 CAN은 차동 신호라 기준 GND 경로가 길어지면 노이즈에 취약해진다. 지상 정지 상태에서는 무해하나, **모터 5개 구동 시 전자기 노이즈가 급증**하므로 그때 배터리 값이 튀거나 CAN이 끊기면 **GND 한 가닥 추가를 1순위로 의심**할 것.

### 동봉 케이블 구성

✅ **분기 케이블은 동봉품이다** (제품 사진 판독, 2026-08-11). 별도 제작/구매가 필요하지 않다.

![PM08 본체와 동봉 케이블](images/04-product-with-bundled-cables.png)

본체(8AWG 전력선 + XT90 + 링터미널 사전 결선) 아래에 **케이블 4종**이 동봉된다:

| # | 구성 | 용도 |
|---|---|---|
| 1·2 | 6핀 ↔ 6핀 (빨강2 / 검정2) | 전원 전용 (CAN 미사용 FC용) |
| **3·4** | 6핀 → **6핀 + 4핀 Y자 분기** (빨강2 / 검정2 + **노랑·파랑**) | **전원 + CAN 분기** ← 본 기체 사용 |

3·4번이 Y자로 갈라져 굵은 4가닥(빨강2/검정2)은 6핀 커넥터로, 얇은 노랑·파랑 2가닥은 별도 4핀 커넥터로 빠진다. **POWER1/POWER2 2계통용으로 2개**가 들어 있다.

> 이 구조가 [FC 연결 제약](#1-커넥터-규격핀-수-불일치--분기-케이블-필요)에서 말한 "PM08 6핀 → (Power 6핀 + CAN 4핀) 분기"를 그대로 구현한 것이다. 즉 Holybro가 처음부터 Pixhawk 표준 FC 연결을 상정하고 케이블을 넣어둔 것.
>
> ⚠️ 동봉 케이블의 CAN 측은 **4핀 커넥터에 노랑·파랑 2가닥만** 들어간다. 즉 **CAN1 핀4(GND)가 비는 것은 케이블 설계상 정상**이며, 결선 실수가 아니다.
