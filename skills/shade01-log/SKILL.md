...
> 🔴 **웹에 보이는 로그를 분석하라고 하면 랩서버에서 받아라** (2026-09-11).
> 원본이 `ku@100.86.239.31:~/shade01-data/logs/` 에 그대로 있다 — FC 에 붙을 이유가
> 없고, 파일명에 로그 번호가 그대로 붙어 있어 경로 매핑도 필요 없다.
> FC MAVFTP 는 **아직 웹에 안 올라간** 로그에만 쓴다
> ([절차](../../tools/qgclog/FETCHING.md#-먼저-웹에-이미-올라간-로그면-랩서버에서-받아라-2026-09-11)).

> 🔴 **파싱이 실패하면 [`PARSING.md`](../../PARSING.md) 를 먼저 읽어라. 고치기 전에.**
> "읽기 실패" 로 보고된 것의 대부분은 파일 문제가 아니었다 — 파일은 멀쩡한데
> 도구·직렬화·전송이 깨진 것이었다(여섯 번 중 다섯 번). 그 문서의 §1 진단 순서를
> 밟으면 어디가 깨졌는지 네 단계로 갈린다. **파일부터 의심하지 마라.**
...
## 🔴 통신 단 — 조종 링크 세기다 (2026-09-12)

로그 뷰어의 **「통신」** 단이 조종 링크 품질을 보여준다. 거리와 겹쳐 그리므로
"멀어서 나빠진 것인가" 를 한 화면에서 가릴 수 있다.

⚠️ `input_rc.rssi` 가 100 고정인 것을 보고 "링크 값이 없다" 고 판단하지 마라 —
PX4 내부 필드일 뿐이고, **ELRS 백팩이 `RADIO_STATUS` 로 실제 값을 보낸다.**
근거: [web/live/README](../../web/live/README.md#-조종-링크-세기는-crsf-없이도-온다-2026-09-12-규명)

| 계열 | 뜻 |
|---|---|
| **기체 RSSI** | 조종기 → 기체 도달 세기. **이것이 조종 링크** (−dBm, 0에 가까울수록 셈) |
| 지상 RSSI | 기체 → 조종기. 91% 가 254 포화라 회색 점선으로만 |
| **노이즈** | 배경 잡음. 낮을수록 좋다 |
| **홈 거리** | 우축. 링크와 겹쳐 봐야 원인이 갈린다 |

「링크 여유」 단의 **송신 버퍼**가 떨어지면 링크가 밀리고 있다는 뜻이다 —
2026-09-09 "Sensor lost" 를 쫓을 때 이 값이 먼저 움직였다.
