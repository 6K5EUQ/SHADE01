...
판정까지 바뀔 수 있다 (`ground` → `unreadable` 실측).

**그래서 이미 받은 파일을 다시 받지 마라.** 스크립트가 알아서 건너뛰고,
업로드도 `rsync --ignore-existing` 이라 서버 정본을 덮지 않는다.

원인·대응은 [FLIGHT-SYNC.md](../../FLIGHT-SYNC.md#-fc-의-sd-읽기가-매번-다른-데이터를-낸다-2026-09-06-발견).
**SD 카드 교체가 필요하다** — 사용자가 로그 손상을 물으면 이걸 알려라.
