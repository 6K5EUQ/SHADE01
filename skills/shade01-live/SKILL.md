...
⚠️ **백팩 쪽은 AP 에 붙은 상태에서 `on` 을 해야 열린다.** 열 주소(`10.0.0.x`)와
포트(백팩의 `send`)를 그때 백팩에게 물어 정하기 때문이다. AP 에 나중에 붙었으면
`on` 을 한 번 돌려라 — 그 뒤로는 오갈 때마다 자동이다.
...
| 언제 | 무엇 |
|---|---|
| 야외에서 arm | `logs/live/<시각>_KST_<경로>.tlog` 를 연다 |
| disarm | 10초 더 받아 적고 닫는다 |
| **실내** arm | **안 적는다** — 하단 바에 `실내대기` 가 뜬다 |

🔴 **야외 판정은 GPS 다** — 3D fix + 위성 6기 이상. 실내 벤치 arm 을 걸러내려고
넣었다. arm 순간에 못 넘었어도 **fix 가 잡히는 순간부터 적기 시작한다.**
...
**「파일이 왜 없나」를 물으면** `status` 가 아니라 화면 하단 바의 기록 칸을
보라고 하라. `실내대기` 면 GPS 를 못 잡은 것이다 (실내이거나, 아직 수렴 전).
