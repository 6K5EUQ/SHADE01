...
2. **`qgclog list` 를 먼저 보여준다** — 사용자가 번호를 고를 수 있게.
   ...
   목록은 이 표를 낸다 — **그대로 옮겨 적어라**:

   ```
   #   파일   시간(KST)   최대고도   최대속도   비행시간   파일크기
   ```

   고도·속도는 `vehicle_local_position` 을 arm 구간으로 잘라 낸 값으로,
   `qgclog <N>` 의 "최고고도 / 최대속도" 와 같은 수치다. 그 토픽이 없으면 `-` 다.
   **고도는 EKF 원점 기준이라 음수가 흔하다** — 이륙 지점 기준이 아니다
   ([해석 시 주의](#해석-시-주의) 참고). 목록만 보고 "고도가 마이너스다" 로
   판정하지 마라.

   표만으로 지상 테스트와 실비행을 가를 수 있다 — **최대속도**가 가장 잘 듣는다
   (지상 0.5 m/s 이하 / 실비행 3 m/s 이상).
