...
**전** `preflight` → **중** `qgc-live` → **직후** `flight-sync` → **끝난 뒤** `qgc-log`.

| 스킬 | 용도 |
|---|---|
| [preflight](preflight/SKILL.md) | **비행 전** — FC·미션·GPS·전원·failsafe 를 한 번에 읽고 GO / NO-GO. `./shade01 test` (읽기 전용, 3~7초) |
...
for s in preflight qgc-log qgc-live flight-sync; do
...
`preflight` 의 기대값은 전부 [FC_CHANGELOG.md](../FC_CHANGELOG.md) 에서 왔다 —
