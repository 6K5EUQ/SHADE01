...
| **「링크 없음」·패킷 0** | 그 포트로 MAVLink 가 안 온다 | 브리지를 같은 PC 에서 돌리면 `pc_bridge.sh` 의 `TARGETS` 에 `127.0.0.1:14551` 이 있어야 한다. 고쳤으면 `systemctl --user restart shade-bridge` |
...
| `rim` | **14551** | 수동 | QGC 가 14550 을 쥔다 |
| `gram-labtop` | 14550 | 수동 | 비어 있다 |

`rim`·`gram` 은 들고 나가는 노트북이라 필요할 때만 켠다.
