...
`systemd --user` 로 돈다 — **터미널을 닫아도 살아 있다.** 부팅 자동시작은
일부러 안 건다: `on`/`off` 가 명시적 스위치다.
...
| `● ON` + `링크: 없음` | 한 번도 프레임을 못 받았다. 포트·경로 문제 |
| `○ OFF` | 꺼져 있다 |
...
| 「링크 없음」이 안 바뀜 | 그 포트로 MAVLink 가 안 온다 | 브리지가 도는지, 백팩 AP 에 붙었는지 |
...
|---|---|---|
| `ku` | 14550 | 비어 있다 |
| `rim3` | **14551** | `shade-bridge` 가 14550 을 쥔다 |
| `rim` | **14551** | QGC 가 14550 을 쥔다 |
| `gram-labtop` | 14550 | 비어 있다 |
