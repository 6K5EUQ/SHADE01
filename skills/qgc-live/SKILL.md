...
### 경로를 잘못 잡았다고 하면 (USB ↔ 백팩 전환)

`on` 의 출력에 나오는 `경로:` 줄이 실제와 다르면 그것부터 본다.

```bash
curl -s 'http://localhost:4400/api/state?track=0' \
  | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get("link"), d.get("src"))'
```

`ELRS 10.0.0.1:...` 이면 백팩, 그 외면 브리지다. `on` 의 `경로:` 줄과 **같아야
한다.** 다르면 버그다 — 아래 이력을 보라.

🔴 **USB 를 뽑고 백팩으로 갈아탔으면 `./qgc live on` 을 다시 한 번 돌려라.**
경로가 바뀌면 들을 포트(`/mavlink` 의 `send`)와 바인딩 주소(WiFi 10.0.0.x)가
같이 바뀌고, 백팩 poker 도 그때 뜬다. 켜 둔 채로 케이블만 바꾸면 트래커는 옛
설정 그대로다.

**고친 이력 (2026-09-06, rim3):** `detect_path()` 가 "프레임이 들어오는 중이면
무조건 USB" 였다. USB 를 뽑고 백팩 AP 로 붙은 뒤 `on` 을 하면 백팩으로 데이터가
멀쩡히 들어오는데도 `경로: USB 직결/브리지` 로 잡고 **poker 를 껐다.** 백팩은
먼저 말을 걸어온 곳에만 보내므로, 남아 있던 세션이 끊기는 순간 조용해졌다.
지금은 트래커가 이미 계산해 둔 `link` 값을 그대로 쓴다.
