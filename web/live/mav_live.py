...
둘 다 살아 있으면 **ELRS 를 쓴다** (`LINK_PRIORITY`, 2026-09-11 에 USB 에서
바꿨다). 날 때 쓰는 링크를 지상에서도 그대로 보기 위해서다 — 근거는 아래
`LINK_PRIORITY` 주석. 우선 경로가 조용해지면 `LINK_FALLBACK_AFTER` 초 뒤
아래 경로가 이어받는다.
...
# 대역폭이 좁아 하트비트 간격이 벌어지므로 넉넉히 잡는다.
LINK_TIMEOUT = 3.0

# 🔴 낡은 실측값 — 이 파일에 있던 「백팩 285 B/s·자세 1.6Hz」 를 지웠다
#    (2026-09-11). 그 값은 **2026-09-06 측정**이고, 당시 FC 는 고장 상태였다:
#    `MAV_0_FORWARD=1` 이라 USB 브리지 트래픽 2736 B/s 가 TELEM1 으로 밀려나가
#    PX4 가 `rate_multiplier` 를 **하한 0.050** 까지 조이고 있었다 (`txbuf` 0).
#    1.6Hz 는 링크의 성능이 아니라 **그 감산의 결과**다.
#
#    2026-09-09 에 원인을 잡았다 (`MAV_0_FORWARD=0`, `extras.txt` 재배분,
#    `MAV_0_RATE` 990→1400). 이후 `rate_multiplier` **1.000** · `txbuf` **100**
#    — 지금은 깎이지 않는다. 설정한 Hz 가 그대로 나간다.
#
#    ⚠️ 그러므로 「설정 Hz 의 1/3만 도착한다」 는 식으로 저 숫자에서 비율을
#       끌어내지 마라. 고장 시절 값으로 현재 성능을 추정하는 것이다 —
#       2026-09-11 에 실제로 그 오류를 한 번 저질렀다.
#       현재 링크 제원은 리포 README 「ELRS」 와 FC_CHANGELOG 9/9 항목이 정본이다.
#
# 두 경로가 동시에 들어올 때 무엇을 화면에 쓸지 — 앞이 강하다.
#
# 🔴 **ELRS 가 우선이다** (2026-09-11). 전에는 USB 였다 — 대역폭이 두 자릿수
#    크다는 이유였는데, 그것은 "화면이 더 낫다" 와 다른 말이었다.
#
#    USB 는 **지상에서만** 붙어 있는 경로다. 비행에 나가면 케이블이 빠지고
#    백팩만 남는다. 그런데 USB 를 우선으로 두면, 지상 점검에서 보던 그림과
#    실제로 날 때 보는 그림이 **다른 링크**에서 나온다 — 조종자가 이륙 직전에
#    익숙해진 갱신 감각이 이륙과 동시에 바뀐다.
#
#    ELRS 를 우선으로 두면 지상에서도 날 때와 **같은 링크**를 본다. 백팩이
#    실제로 몇 Hz 를 주는지, 무엇이 늦는지를 이륙 전에 이미 보고 있으므로
#    링크 문제를 지상에서 잡는다. USB 는 백팩이 조용할 때만 이어받는다.
#
#    대역폭 걱정은 실측으로 접혔다 (2026-09-11, FC_CHANGELOG): 백팩으로도
#    설정 Hz 의 **94%** 가 도착하고 `BAD_DATA` 는 0 이다. 자세 9.4Hz —
#    화면이 부족함을 느낄 속도가 아니다.
LINK_PRIORITY = ('ELRS', 'USB')
...
class Recorder:
    ...
    🔴 **수신 경로마다 파일을 따로 연다** (ELRS 백팩 / FC USB 브리지). 둘은
       대역폭도 갱신 주기도 다르고(⚠️ 아래 「낡은 실측값」 참조), 한 파일에
       섞으면 어느 링크가 무엇을 놓쳤는지 못 가린다 —
...
class State:
    ...
    def _active_locked(self, now=None):
        ...
        조종자가 경로를 고정해 두었으면 그것을 먼저 따른다. 고정한 경로가 죽어
        있어도 유지한다: 조용하다는 사실 자체가 보려던 정보이고, 말없이 다른
        경로로 넘어가면 그것을 못 본다.

        ⚠️ 고정 기능은 원래 "USB 우선이라 ELRS 를 영영 못 본다" 를 푸는 손잡이
        였다. 2026-09-11 에 기본이 ELRS 로 바뀌어 그 이유는 사라졌지만, 반대
        방향(백팩이 붙어 있는데 USB 를 보고 싶다)으로 여전히 쓴다.
        """
...
def _link_kind(src):
    ...
    보낸 쪽 주소로 가른다. 둘은 대역폭도 갱신 주기도 달라서 지금 무엇을
    보고 있는지가 화면에 드러나야 한다 (⚠️ 수치는 「낡은 실측값」 참조).
...
def handle(msg, st):
    ...
    if t == 'HEARTBEAT':
        ...
        #    MAIN3/4/6/7 = VTOL 우후/우전/좌후/좌전.
        #
        # 🔴 여기 숫자는 MAIN 핀 번호 그대로다. `SERVO_OUTPUT_RAW` 의 필드는
        #    `servo1_raw` 부터라 **1 부터 센다** — MAIN3 이 곧 servo3_raw 다.
        #    재생 쪽(playback.py)은 ULog `actuator_outputs` 를 읽는데 그쪽
        #    `output[]` 은 0 부터라 같은 핀이 2,3,5,6 이다. 두 파일의 숫자가
        #    다른 것은 그래서이고, 둘 다 맞다.
        #    ⚠️ 여기를 2,3,5,6 으로 "고치지" 마라. 그러면 MAIN2(좌 에일러론)
...
def receiver(sock, st, alive=None):
    ...
    while True:
        ...
        with st.lock:
            ...
            #    두 링크가 서로 덮어써 고도·자세가 튄다 (갱신 주기가 다르다).
