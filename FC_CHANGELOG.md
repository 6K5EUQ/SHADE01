...
🔴 **같은 오프셋으로 +2.21 과 −0.93 이 나왔다.** 이 절은 원인을 "온도·기압" 으로
추정해 뒀는데, **9/13 에 갈렸다 — 실내 기류(에어컨)다.** 위 정정 상자를 보라.
두 측정 다 실내였고, 그날그날 송풍 상태가 달랐던 것이다.

**남는 결론은 같다: 실내 측정으로는 오프셋을 판정할 수 없다.** 야외 무풍
재측정 전까지 미판정이다.
