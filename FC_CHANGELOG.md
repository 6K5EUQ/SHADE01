...
| 2026-09-11 저녁 (시각 미상) | **조종자** | **(미상)** | `SENS_DPRES_OFF` | `-1.8096` → **`-3.0643`** | 에어스피드 재보정. ~~과보정~~ ⚠️ **그 판정은 철회한다 (2026-09-13)** — 근거였던 「정지 +2.21 m/s」는 **실내에서 잰 값**이고 에어컨 바람이었다. 야외 무풍 재측정 전까지 미판정. 아래 절 |
...
| 2026-09-11 10:32 | `ku` | `rim3` USB 직결 (MAVFTP) | `extras.txt` 스트림 4종 | EKF `0.01`→**`2`** / 진동 `0.01`→**`2`** / 배터리 `4`→**`5`** / `VFR_HUD` `0.01`→**`2`** | 남은 예산을 **보조자 화면**에 썼다. 실측 738→**936 B/s** (예산 67%). 아래 절 |
...
그중 **실제로 사라진 것은 12개**다. 전부 "꺼짐" 값이라 동작이 같다:

`MNT_MODE_IN=-1` · `RC_CRSF_PRT_CFG=0` · `SEP_PORT1/2_CFG=0` ·
`TC_A/B/G/M_ENABLE=0` · `UXRCE_DDS_CFG=0` · `SYS_CAL_ACCEL/BARO/GYRO=0`

⚠️ **`MIS_DIST_1WP` 은 사라지지 않았다.** 되읽기 시점에 아직 인스턴스화되지
않아 목록에 안 잡혔을 뿐이고, 나중에 다시 뜨니 `10000.0` 으로 있다. 파라미터
개수도 그래서 **1354 − 12 = 1342** 로 맞는다 (실측 확인, 2026-09-12 19:2x).
...
🔴 **변경 자체는 내가 하지 않았고 기록도 없었다.** 조종자가 "보정했다" 고 알려 줘
검증만 했다. **누가·언제·어느 PC 로 썼는지 남아 있지 않다** — 이 문서가 있는 이유가
바로 그것이니, 다음부터는 쓴 사람이 그 자리에서 남겨야 한다.

#### 실측 (2026-09-11, `rim3` USB 직결, 지상 정지, 30초 × 2회)

| | 이전 기록 | **1차** | **2차** |
|---|---|---|---|
| `airspeed` 평균 | -4.7 ~ -5.0 m/s | **-1.233** | **-1.412** |
| `airspeed` 최소 | — | -1.719 | -1.830 |
| 표준편차 | — | 0.192 | 0.156 |
| `press_diff` 평균 | — | **-0.027 hPa** | **-0.030 hPa** |
| `groundspeed` | — | 0.003 | 0.003 (정지 확인) |

**README 「다음 비행 전」 2번의 ±2 m/s 기준 충족.** 두 번 재서 재현됐다
(표준편차 0.16~0.19 라 잡음이 아니라 실제 편향).

#### 🔴 남은 음수는 정상이다 — 세 가지 근거

**1. PX4 가 음수를 일부러 보존한다.** `src/lib/airspeed/airspeed.cpp` 의 `calc_IAS`:
...
0 으로 자르지 않는다. 역류·포트 반대 연결을 감지하려고 **설계상 부호를 남긴다.**

**2. 잔차 2.8 Pa 는 센서 분해능 급이다.** MS5525DSO 정밀도가 ±1~2 Pa 다.
√ 때문에 0 부근에서만 크게 보인다 — 같은 2.8 Pa 가 속도별로:

| 속도 | 이 오차의 영향 |
|---|---|
| 0 m/s (지상) | **-1.3 m/s** ← 지금 보는 것 |
| 7 m/s (`FW_AIRSPD_STALL`) | 약 -0.2 m/s |
| 15 m/s (`FW_AIRSPD_TRIM`) | 약 -0.09 m/s |

**3. 🔴 같은 FC 로 실제 천이한 기체가 우리보다 더 음수다.**
서버의 참조 로그 `logs/REF_vtol_mission_v1170_6C_2025-11-25.ulg`
(**`PX4_FMU_V6C` — 우리와 같은 하드웨어**, VTOL 미션 306초, MC→FW→MC 완주):

| 구간 | IAS 평균 | 최소 | **음수 비율** |
|---|---|---|---|
| **MC (호버·저속)** | **-0.27 m/s** | **-7.33** | **54.5%** |
| FW (고정익 순항) | +20.29 | +15.23 | **0.0%** |

MC 구간 `diff_pres` 평균 **-3.03 Pa**, 음수 비율 66.7%.
**우리(-2.8 Pa / -1.3 m/s)가 더 얌전하다.** 그리고 그 기체는 그 상태로 천이에 성공했고,
FW 구간 172 샘플 중 음수가 **하나도 없다** — 저속에서만 나타나는 현상임이 실측으로 확인된다.

| | 우리 | REF (실비행) |
|---|---|---|
| `SENS_DPRES_OFF` | **-1.8096** | **-1.3411** |
| `ASPD_PRIMARY` | 1 | 1 |
| `ASPD_DO_CHECKS` | 7 | 7 |
| `ASPD_SCALE_APPLY` | 2 | 2 |

오프셋 차이 0.47 Pa. **설정이 사실상 같다.**

**결론: 영점 재보정은 필요 없다.** 지금 값으로 둔다.

#### 🟡 진짜 남은 것은 `ASPD_SCALE_1` 이다 — 비행해야 잡힌다

| | 우리 | REF |
|---|---|---|
| `ASPD_SCALE_1` | **1.0** (미보정) | **1.0742** (학습됨) |

영점은 **압력 0 점**이고, 스케일은 **비례 오차**(피토관 설치 위치·기류 교란)다.
둘은 다른 것이고 **스케일은 지상에서 잴 수 없다.**

자동 학습 조건은 이미 다 갖춰져 있다:

| 파라미터 | 우리 값 | 뜻 |
|---|---|---|
| `ASPD_SCALE_1` | **1.0** | 정확히 1.0 이면 PX4 가 **첫 5분간 학습을 가속**한다 |
| `ASPD_SCALE_APPLY` | **2** | **비행 중(in-air) 적용.** 1(착륙 시 적용)보다 안전 — 영점이 나쁜 채로 시작해도 공중에서 바로잡는다 ([PR #19787](https://github.com/PX4/PX4-Autopilot/pull/19787)) |
| `ASPD_SCALE_NSD` | 1e-4 | 추정기 잡음밀도 (기본값) |

⚠️ **단, 고정익 구간이 있어야 학습한다.** 추정은 GNSS 속도와 대기속도를 맞춰 푸는
것이라 **MC(쿼드) 비행으로는 안 잡힌다.** 지금은 [고정익 금지](../README.md#-고정익-사용-금지--쿼드-전용-2026-09-04)라
`RC_MAP_TRANS_SW=0` — **천이를 풀기 전까지 스케일은 1.0 그대로다.**

⚠️ **선회가 필요하다.** 검증된 스케일은 **12개 방위 구간(30°씩)** 을 채워야 갱신된다.
직선만 날면 바람과 스케일이 구분되지 않는다. 첫 고정익 비행은 **원선회를 넣어라.**

#### 속도 임계는 REF 것을 베끼지 마라

| | 우리 | REF |
|---|---|---|
| `FW_AIRSPD_STALL` | 7.0 | **14.0** |
| `FW_AIRSPD_TRIM` | 15.0 | **18.0** |
| `FW_AIRSPD_MIN` | 10.0 | **16.0** |
| `VT_ARSP_TRANS` | 10.0 | **16.0** |

REF 는 훨씬 빠른 기체다. 우리 값은 **PX4 공장 기본값 그대로**이고,
9/5 항목의 판단(실측 전에는 남의 숫자를 넣지 않는다)을 그대로 유지한다.

### ✅ 2026-09-11 10:32 — 남은 예산을 보조자 화면에 (EKF·진동 되살림)

**작업**: `rim3` USB 직결 MAVFTP → 재부팅 → 되읽기 검증 → ELRS 실측. **DISARM** 확인.
백업 **1730 B crc32 `3216b3f4`** → 업로드 **1721 B crc32 `04e8a2b6`** → 되읽기 **일치**.

#### 왜 — 비행은 항상 2인이다

조종자는 **육안으로** 기체를 보며 수동조종한다. 화면을 볼 여유가 없다.
보조자가 실시간 로그 화면을 본다. **그러므로 텔레메트리 예산의 수요자는
조종기(EdgeTX 센서)가 아니라 웹 뷰어다.** 백팩 경로는 CRSF 12종 화이트리스트를
안 거치고 MAVLink 원본을 받으므로 조종기에 없는 값도 웹에서는 보인다.

판단 기준: **"보조자가 무엇을 보고 조종자에게 경고해야 하나."**

| 스트림 | 이전 | 이후 | 실측 증가 | 왜 |
|---|---|---|---|---|
| `ESTIMATOR_STATUS` | 0.01 | **2** | +101 B/s | 🔴 **EKF 차트가 소스 없이 비어 있었다.** 혁신비율 1.0 초과는 위치·속도·고도 추정이 발산 중이라는 **유일한 조기경보**다. 육안 조종자는 기체가 이상해진 뒤에야 안다 |
| `VIBRATION` | 0.01 | **2** | +58 B/s | 프로펠러 손상·마운트 풀림이 먼저 나오는 곳. 보조자가 **비행 중에** 착륙을 권할 수 있다 |
| `BATTERY_STATUS` | 4 | **5** | +43 B/s | 9/5 의 81.2 A 첨두가 **0.20 초** 지속 — 4Hz(0.25s)로는 아슬아슬했다 |
| `VFR_HUD` | 0.01 | **2** | +54 B/s | HUD 의 `airspeed` 출처 |

🔴 **진동 억제는 판단이 틀렸던 것을 되돌린 것이다.** 같은 날 아침에 "진동은
실시간 판단 대상이 아니라 `.ulg` 로 본다" 며 껐는데, **그것은 1인 운용 전제**였다.
2인이면 보조자가 급증을 보고 내리게 할 수 있고, 사후 분석은 이미 떨어진 뒤다.

#### ELRS 실측 (90초, 지상 정지) — 넷 다 도착한다

| 스트림 | 설정 | **실측** | 도달률 |
|---|---|---|---|
| `ESTIMATOR_STATUS` | 2 | **1.86 Hz** | 93% |
| `VIBRATION` | 2 | **1.81 Hz** | 91% |
| `VFR_HUD` | 2 | **1.86 Hz** | 93% |
| `ATTITUDE` | 10 | **9.41 Hz** | 94% (유지) |
| `GPS_RAW_INT` | 3 | **2.88 Hz** | 96% (유지) |

`BATTERY_STATUS` 는 **배터리 미연결이라 안 잡혔다** — 다음 비행에서 5 Hz 확인할 것.

| | 이전 | **이후** |
|---|---|---|
| 총 트래픽 | 738 B/s | **936 B/s** |
| `MAV_0_RATE=1400` 대비 | 53% | **67%** |
| OTA 상한 1651 대비 | 45% | **57%** |
| `BAD_DATA` | 0 | **0** |

**여유 464 B/s 남았다.**

#### 같이 바꾼 것 — 뷰어 기본 차트 (조종자 지시)

스트림만 올리면 차트가 기본 off 라 아무도 안 본다. 그래서 함께 고쳤다:

- **「고도」와 「속도」를 한 칸으로 합쳤다** (`k-alt`, 제목 **고도+속도**).
  상승률은 뺐다 — 고도선의 기울기가 같은 것을 말한다. 속도는 **GPS 속도**다.
- **`k-spd` 단을 없앴다.** 피토관 선도 같이 빠졌다 — 이 기체는 **고장품**이라
  정지 시 −4.7~−5.0 m/s 다 (`SENS_DPRES_OFF=-4.52`). 영점을 고치면 되살려라.
- **기본으로 뜨는 것은 「고도+속도」와 「자세」 둘뿐이다.** 진동·EKF·전력·GPS 는
  필요할 때 켠다.

⚠️ **`VFR_HUD` 를 2Hz 로 올렸지만 피토관 차트는 뺐다.** `airspeed` 는 HUD 판독에만
쓰인다. 에어스피드를 안 고칠 거면 `VFR_HUD` 는 **0.01 로 되돌려 54 B/s 를 아껴도 된다.**

#### 함정 — `reboot.py` 가 pymavlink 버그로 죽었다

첫 재부팅 시도가 `TypeError: 'NoneType' object does not support item assignment`
(`mavutil.py:98`) 로 죽었다. **pymavlink 2.4.49 의 알려진 버그**로
[FETCHING.md](../tools/qgclog/FETCHING.md#pymavlink-2449-버그-우회) 에 이미 적혀 있던 것이다.
`try/except TypeError` 로 감싸 다음 패킷으로 넘어가게 고쳤다 (`tools/fc/reboot.py`).
**업로드는 그 전에 끝나 있었다** — 재부팅만 다시 하면 됐다.
...
#### ✅ ELRS 실측 — 설정대로 온다 (2026-09-11 10:12 · 10:19, 지상 정지)

백팩 경로(`10.0.0.100:14550` ← `10.0.0.1:14555`)로 **90초씩 두 번** 셌다.
`shade-live` 를 잠시 멈추고 그 소켓을 직접 받았다 (측정 후 복구).

| 스트림 | 설정 | 1차 | **2차** | 도달률 |
|---|---|---|---|---|
| `ATTITUDE` | 10 | 9.46 | **9.42 Hz** | **94%** |
| `GPS_RAW_INT` | 3 | 2.90 | **2.80 Hz** | 93% |
| `HEARTBEAT` | 2 | 1.81 | **1.90 Hz** | 95% |
| `SERVO_OUTPUT_RAW_0` | 2 | 1.87 | **1.88 Hz** | 94% |
| `SYS_STATUS` | 0.5 | 0.49 | **0.47 Hz** | 94% |
| `HOME_POSITION` | 0.1 | 0.09 | **0.10 Hz** | 100% |
| `VIBRATION` | 0.01 | 0.01 | **0.01 Hz** | 억제 성공 |
| **총 트래픽** | | 739 | **738 B/s** | |
| **`BAD_DATA`** | | 0 | **0** | |

**두 번 재서 총량 1 B/s 차이 — 안정됐다.**

| | 계산 | **실측** |
|---|---|---|
| 총 트래픽 | 1145 B/s | **738 B/s** |
| `MAV_0_RATE=1400` 대비 | 82% | **53%** |
| ELRS OTA 상한 1651 대비 | 69% | **45%** |

실측이 계산보다 **407 B/s 낮다.** PX4 가 페이로드 뒤쪽 0 바이트를 잘라 보내기
때문이다 (계산은 full payload 최악값). **예산 여유가 예상보다 크다.**

🔴 **「설정 Hz 의 1/3만 도착한다」 는 추정은 틀렸다.** 94% 가 온다. 그 추정의
근거였던 9/6 의 1.6Hz 는 고장 상태(`rate_multiplier` 0.050) 값이었다 —
이 문서 위쪽 「낡은 실측값」 경고와 같은 건이다. **자세는 1.6 → 9.42 Hz, 5.9배.**

⚠️ **지상 정지 기준이다.** 비행 중 거리가 멀어지면 `txbuf` 가 떨어져 PX4 가
깎을 수 있다. 45% 여유가 그 보험이다. 2차에서 `SYS_STATUS` 최대공백이
3.94 → **6.06초** 로 늘었고 같은 순간 `UNKNOWN_436/411` 도 6.06 을 찍었다 —
한 번 몰려 밀린 흔적이다. EdgeTX 20초 임계에는 3배 여유지만 **비행 중 이 공백이
커지는지 봐야 한다.**

🟡 **`shade-live` 를 재시작하면 `pin` 이 auto 로 풀린다.** 화면에서 ELRS 를 계속
보려면 데이터 원천 배지를 다시 눌러 고정해야 한다.

**목적**: 웹 실시간 화면의 자세 갱신을 높인다. 브라우저 폴은 20Hz, 차트 격자는 5Hz
인데 소스가 5Hz 라 그 위로는 의미가 없었다.

#### 변경 (TELEM1 = `/dev/ttyS5` = ELRS)

| 스트림 | 이전 | 이후 | B/s 증감 | 왜 |
|---|---|---|---|---|
| `ATTITUDE` | 5 | **10** | **+200** | 목적. 웹 자세 갱신 |
| `VIBRATION` | 5 | **0.01** | **-220** | 실시간 판단 대상이 아니다. 느린 추세값이고 이상은 `.ulg` 로 본다. 웹 차트도 기본 off |
| `SERVO_OUTPUT_RAW_0` | 5 | **2** | **-147** | 웹 차트 기본 off. 5Hz 로 받아 아무도 안 봤다 |
| `HEARTBEAT` | 3 | **2** | **-21** | 모드 표시용. EdgeTX 임계 20초에 2Hz 면 40배 여유 |
| `HOME_POSITION` | 0.5 | **0.1** | **-29** | **홈 좌표는 안 변한다.** ARM 때 한 번 잡히고 끝인데 2초마다 72B(최대 프레임)를 받고 있었다 |
| **합** | | | **-216** | |

**안 건드린 것**: `BATTERY_STATUS` 4Hz (9/5 의 81.2A 첨두가 **0.20초 지속** — 낮추면
못 잡는다), `GPS_RAW_INT` 3Hz (이 기체는 `GLOBAL_POSITION_INT` 가 안 올 때가 있어
실측 201:0 — 항적의 실질 유일 소스), `GLOBAL_POSITION_INT` 3Hz (고도·상승률·GPS속도가
여기서 나오고 **기본 ON 차트 두 개**가 그것을 쓴다), `STATUSTEXT` 5Hz (이벤트성이라
평소 0 B/s), `SYS_STATUS` 0.5Hz (저비용).

#### 예산

MAVLink v2 프레임(오버헤드 12B + 페이로드)으로 계산한 **수요**다. 실측 `tx` 가 아니다.

| | 이전 | 이후 |
|---|---|---|
| 스트림 수요 | 1362 B/s | **1145 B/s** |
| `MAV_0_RATE=1400` 대비 | **97%** | **82%** |
| ELRS OTA 상한 1651 B/s 대비 | 82% | **69%** |

**자세를 2배로 올리면서 총량은 줄었다.** 97% 는 `rate_multiplier` 감산이 시작되는
선에 붙어 있던 값이라, 82% 는 비행 중 `txbuf` 가 흔들릴 때의 여유이기도 하다.

#### 🔴 `-r 0` 이 아니라 `-r 0.01` 이다

진동을 "끈다" 고 `-r 0` 을 쓰면 **반대로 무제한(`inf`)으로 켜진다.**
`configure_stream()` (`mavlink_main.cpp:1155-1194`) 이 목록에 없는 이름을 만나면
새로 만들면서 `interval=0` 을 주는데 그게 곧 무제한이다. 9/9 1차 시도에서
`ODOMETRY`(245 B/프레임)가 이렇게 켜진 이력이 있다. 억제군 20종이 전부 `0.01`
(100초 주기 = 실효 0.004 Hz)을 쓰는 이유다.

⚠️ **웹 진동 차트(`k-vib`)가 죽는다.** 기본 off 라 평소엔 안 보이지만 켜도 선이
안 그려진다. `.ulg` 에는 그대로 기록되므로 비행 후 분석은 영향 없다.

#### 곁다리 — 리포 안의 낡은 실측값을 지웠다

`mav_live.py`·`live/README.md`·`live.js` 9곳이 「백팩 285 B/s · 자세 1.6Hz」 를
현재 성능처럼 인용하고 있었다. **그 값은 2026-09-06 측정이고 당시 FC 는 고장
상태였다** — `MAV_0_FORWARD=1` 로 `rate_multiplier` 가 하한 0.050 에 붙어 있었다.
1.6Hz 는 링크 성능이 아니라 그 감산의 결과다.

이번 세션에서 **그 숫자로 "설정 Hz 의 1/3만 도착한다" 는 비율을 끌어내는 오류를
...
TELEM1 에 한 번도 적용되지 않는다. 9/6 세션 최대 함정이었다.

**검증할 것 (다음 야외 비행)**:
- `ATTITUDE` 가 실제로 10Hz 로 도착하는가. 비행 중 `rate_multiplier` 가 1.000 을
  유지하는가 — `MAV_0_RADIO_CTL=1` 이라 링크가 나빠지면 PX4 가 자동으로 깎는다.
  **지금 계산은 전부 지상 정지 기준이다.**
- 조종기 "Sensor lost" 재발 여부. `HOME_POSITION` 0.1Hz(10초)는 EdgeTX 임계 20초에
  2배 여유뿐이다 — CRSF 12종에 없어 센서로 등록되지는 않지만 확인할 것.
- `BAD_DATA` 가 늘어나는가.

⚠️ **되돌릴 값은 9/9 판이다** — `git show HEAD:config/extras.txt`.
...
**`extras.txt` 사본도 리포에 넣었다** — [`config/extras.txt`](config/extras.txt) (FC 에서
MAVFTP 로 받은 1726 B, 32줄, 9/9 기록과 바이트 일치). `.plan`·`.params` 는 백업이 있는데
TELEM1 스트림 구성만 FC 의 SD 에만 있었다. SD 를 갈거나 FC 를 바꾸면 이 파일을 되올린다.
...
> [정정 절](#-2026-09-09--com_fltmode1-4--3-hold--mission) 참조.
...
### ⚠️ 2026-09-11 저녁 — 에어스피드 재보정 (`SENS_DPRES_OFF` -1.8096 → -3.0643)

> 🔴 **2026-09-13 정정: 아래의 「과보정」 판정을 철회한다.**
> 근거였던 「정지 상태 +2.21 m/s」는 **실내에서 잰 값**이었고, 조종자가 지적한 대로
> **에어컨 바람**을 피토관이 읽은 것이다. 2.21 m/s 는 실내 송풍 수준이고, 흔들림이
> 0.35 m/s 로 일정했던 것도 기류로 설명된다 — 센서 결함의 징후가 아니다.
> **야외 무풍에서 다시 재기 전까지 이 오프셋은 미판정이다.**
> 실내 조회값으로 판정하지 않는 원칙은 [`CLAUDE.md`](CLAUDE.md#-값을-뽑을-때--실내-조회값과-실비행값을-섞지-마라) 에 넣었다.
>
> 참고로 **9/11 실비행 로그**에서 뽑은 값은 다르다: 정지 구간(GPS<0.5m/s) 영점이
> 세션 초 −3.8 에서 세션 말 −2.9 로 **개선**됐고, 움직일 때 GPS 와 상관 0.77~0.79 로
> **센서는 살아 있다**. 남은 것은 배율(`ASPD_SCALE_1=1.0`, 기울기 실측 1.15)이다.

<details><summary>아래는 철회된 당시 기록 (실내 측정 기반)</summary>


조종자가 "에어스피드 보정했다, 이제 사용 가능하다" 고 알려 와 실측했다. **영점은
오히려 나빠졌다.**

| 시점 | `SENS_DPRES_OFF` | 정지 대기속도 | 판정 |
|---|---|---|---|
| 9/10 이전 | `-4.52` | **-5.0** | 고장 |
| 9/11 낮 검증 | **`-1.8096`** | **-1.3** | ✅ ±2 기준 충족 |
| **9/11 저녁** | **`-3.0643`** | **+2.21** | ⚠️ 과보정·부호 반전 |

실측: 15초 151표본, 최소 `2.03` · 중앙 `2.24` · 최대 `2.38` · 평균 `2.21`
(흔들림 0.35 m/s — 센서 자체는 안정적이다, 영점만 틀어졌다).

#### 과보정이라는 근거

정지 상태의 `+2.21 m/s` 를 차압으로 되돌리면 (`IAS = sign(dp)·√(2|dp|/ρ)`, ρ=1.225):

```
dp = 1.225 × 2.21² / 2 = 2.99 Pa
```

현재 오프셋이 `3.06 Pa` 다. **뺀 만큼이 거의 그대로 양수로 나오고 있다** — 영점을
1.25 Pa 더 뺀 것이 고스란히 양의 편향이 됐다. `-1.8096` 이 ±2 기준 안이었으므로
**그 값이 더 낫다.**

#### 🔁 2026-09-12 재측정 — 값이 뒤집혔다 (같은 오프셋인데 −0.93 m/s)

펌웨어 교체 뒤 실내에서 다시 쟀더니 **9/11 저녁과 정반대**가 나왔다.
`SENS_DPRES_OFF` 는 `-3.0643` 그대로다.

| | 9/11 저녁 | **9/12 실내** |
|---|---|---|
| `VFR_HUD` 대기속도 | +2.21 (151표본) | **−0.93** (154표본, −1.43 ~ +0.22) |
| `airspeed.indicated` | — | **−0.79 m/s** |
| 원시 차압 | — | **−3.447 Pa** |
| 센서 온도 | — | **34.11 °C** |
| 판정 | ⚠️ 과보정 | **±2 기준 충족** |

계산이 맞는다: `-3.447 − (-3.064) = -0.383 Pa` → `-0.79 m/s`.

🔴 **같은 오프셋으로 +2.21 과 −0.93 이 나온다.** 오프셋이 문제가 아니라 **차압
자체가 조건에 따라 3 Pa 넘게 움직인다**는 뜻이다. 온도(34 °C)·기압·실내외 차이가
후보지만 **아직 안 갈랐다.**

**그래서 판단이 바뀐다** — "9/11 낮의 `-1.8096` 으로 되돌려라" 는 처방은 근거가
약해졌다. 그 값도 그날 그 조건의 측정일 뿐이다. **온도를 바꿔 가며 차압 드리프트를
재기 전에는 어떤 오프셋도 정본이 아니다.**

#### 🔴 그래서 고정익은 계속 금지다

조종자 지시로 금지를 유지한다. 실측도 같은 방향이다:

- **양수 오차가 음수보다 위험하다.** PX4 는 음수를 보수적으로 다루지만, 양수는
  **실제보다 빠르다고 착각**하게 만든다. `FW_AIRSPD_STALL=7` / `FW_AIRSPD_MIN=10`
  인데 정지 중 +2.21 이면, 실제 7 m/s 를 9 m/s 로 읽어 **실속 경보를 놓친다.**
- `ASPD_SCALE_1` 은 여전히 **1.0(미보정)** 이다. 비례 오차는 고정익 비행으로만
  학습되고, 실속 판단을 실제로 좌우하는 것은 이쪽이다.

진입 경로 넷은 그대로 닫혀 있다 — 실측으로 확인했다:
`RC_MAP_TRANS_SW=0` · `VT_ELEV_MC_LOCK=1` · 미션 이착륙 `22`/`21` · 6슬롯에 천이 없음.

#### 되돌리려면

`SENS_DPRES_OFF` 를 `-1.8096` 으로 쓰고 정지 상태에서 30초 × 2회를 다시 재라
(±2 m/s 안이어야 한다). 그 값은 9/11 낮에 참조 기체 로그와 대조해 검증한 값이다.

</details>

### 2026-09-11 15:33 — 미션 교체: 5포인트 사각 → 이륙·5초 호버·착륙

미션 테스트용 최소 구성이 필요해서 바꿨다. 기존 것은 지운 게 아니라
[`missions/01-square-5pt.plan`](missions/01-square-5pt.plan) 으로 리포에 남아 있고,
FC 에서 받은 백업은 rim3 의 `~/fc-backup/mission.plan.fcbak-20260911-153248` 이다.

| | 전 | 후 |
|---|---|---|
| 항목 수 | 6 | **3** |
| 구성 | TAKEOFF → WAYPOINT ×4 → LAND | **TAKEOFF → LOITER_TIME 5s → LAND** |
| 이동 | 163.6 m / 약 77초 | **없음 — 제자리 수직** |
| 이착륙 명령 | 22 / 21 | 22 / 21 (그대로) |

좌표는 기존 미션의 이륙점 `35.1810871, 128.5538216`, 고도 5 m 로 셋 다 같다.

**5초 호버는 `LOITER_TIME`(19) 의 param1 로 적었다.** 멀티콥터는 그 점에서 정지해
기다린다 — 선회 경로는 `mission_block.cpp:225` 에서 `VEHICLE_TYPE_FIXED_WING`
일 때만 탄다. `WAYPOINT`(16)+`time_inside` 로도 되지만 그건 `ROTARY_WING` 조건이
붙어 있어, 나중에 천이를 풀면 호버가 조용히 사라진다.

#### 어떻게 올렸나

새로 만든 [`tools/fc/mission_push.py`](tools/fc/mission_push.py) 로 올렸다.
`extras_push.py` 와 같은 뼈대다 — ARM 검사 → 백업 → 검사 → 올리기 → 되읽기.

🔴 **미션에는 CRC 가 없다.** MAVFTP 가 아니라 MISSION 프로토콜로 오가기 때문이다.
`MISSION_ACK` 는 "받았다" 일 뿐 "같다" 가 아니어서, 올린 뒤 **전량을 되읽어 항목마다
비교**하는 것이 유일한 확인이다. 명령·프레임·param1·위경도(1e7 정수)·고도를 대조해
전부 일치했다.

`verify` 는 펌웨어가 거부할 구성을 미리 잡는다 (근거는 `FeasibilityChecker.cpp`):
84·85·3000 이 있으면 거부, 착륙으로 시작하면 거부, `MIS_TKO_LAND_REQ=2` 라 착륙이
없으면 거부. 84 를 넣은 가짜 plan 으로 실제로 막히는 것을 확인하고 올렸다.

#### 함정 둘

⚠️ **웹 트래커의 「미션 항목 0」을 믿고 "FC 가 비었다" 고 판단했는데 틀렸다.**
트래커는 미션을 요청하지 않는다. 실제로는 6항목이 들어 있었고, `backup` 으로
받아 보고서야 알았다. FC 의 미션을 확인할 때는 트래커가 아니라 `backup` 을 써라.

⚠️ **pymavlink `add_message()` 가 터진다** — `messages[mtype]._instances` 가 None
인데 인스턴스 필드가 있는 메시지가 오면
`TypeError: 'NoneType' object does not support item assignment` (mavutil.py:98).
미션을 받다 말고 죽는다. 라이브러리는 건드리지 않고 `mission_push.py` 안에서
`add_message` 를 감싸 막았다 — venv 는 언제든 되돌려지는 자리라서다.
