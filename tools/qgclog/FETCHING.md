...
## 🔴 먼저: 웹에 이미 올라간 로그면 **랩서버에서 받아라** (2026-09-11)

[shade01.bewe.co.kr](https://shade01.bewe.co.kr) 에 보이는 로그는 **랩서버에 원본이
그대로 있다.** FC 에 붙을 이유가 없다.

```bash
# 1. 웹 카탈로그로 무엇이 있는지 본다 (UA 를 바꿔야 한다 — Cloudflare 가 기본 UA 를 막는다)
curl -s -A 'Mozilla/5.0' 'https://shade01.bewe.co.kr/api/logs?all=1' | python3 -m json.tool | head -40

# 2. 랩서버에서 바로 받는다
scp 'ku@100.86.239.31:~/shade01-data/logs/*2026-9-11*.ulg' /tmp/analyze/
```
...
| | 랩서버 | FC MAVFTP |
|---|---|---|
| FC 포트 | **안 건드린다** | 브리지를 매번 stop/start 해야 한다 |
| 속도 | scp — 10편 수 초 | 편당 8~24초 |
| 파일명 | `log_339_2026-9-11-17-25-34.ulg` — **로그 번호가 그대로** | `08_23_14.ulg` — 시각뿐, 번호 매핑이 따로 필요 |
| 위험 | 없음 | 브리지 복구를 빼먹으면 다른 PC 에서 기체가 사라진다 |

⚠️ **랩서버에는 venv 가 없다.** 거기서 파싱하려 하지 말고 파일만 받아 분석 PC 에서 돌려라
(실측: `ModuleNotFoundError: No module named 'numpy'`).

**FC MAVFTP 는 아직 웹에 안 올라간 로그에만 쓴다.** 비행 직후 회수가 그 경우다 —
`./qgc sync` 가 그 흐름이고, 아래 절차가 그 속을 설명한다.
