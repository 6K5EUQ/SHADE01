...
셋 중 하나라도 비면 화면은 뜨고 **점검만 막힌다** (`/api/health` 가
`preflight: disabled` 로 답한다).

## 🔴 암호 둘은 서로 다른 것이다

| | 쓰는 곳 | 왜 따로인가 |
|---|---|---|
| `UPLOAD_PASSWORD` | 로그 업로드 | 이 사이트에 로그를 **영구히** 남긴다 |
| `PREFLIGHT_PASSWORD` | 점검 화면 | 그때뿐인 조회. 현장에서 자주 친다 |

하나로 묶으면 점검용으로 짧게 바꾸는 순간 업로드까지 같이 약해진다. 그래서
`server.js` 가 두 값을 따로 읽고, 업로드 암호로 점검을 부르면 401 이다.

⚠️ **현재 값은 `7777` 이다** (2026-09-20, 사용자 지정). 4자리 숫자라 공개 URL
에서 초 단위로 맞힐 수 있다. 점검은 읽기 전용이지만 **실제 FC 링크를 점유**
하므로, 조종 중에 누가 반복해서 누르면 링크를 나눠 쓰게 된다. 연타는
에이전트가 5초 간격으로 막고 웹서버가 동시 실행을 하나로 제한한다 — 그것이
이 암호 길이에 대한 유일한 완화책이다.
