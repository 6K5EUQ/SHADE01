...
> ⚡ **비행 직후라면 손으로 하지 마라 — [`./qgc sync`](FLIGHT-SYNC.md) 한 줄이다.**
> 브리지 정지·복구, 크기 게이트, 판정, 업로드, 서비스 재시작까지 다 한다.
> 아래는 그 안에서 무슨 일이 도는지와, 손으로 해야 할 때의 절차다.

## 1. 로그 수집

> 🟢 **이미 웹에 올라간 로그를 다시 보려는 것이라면 FC 를 만지지 마라** (2026-09-11).
> 원본이 랩서버에 그대로 있다:
>
> ```bash
> curl -s -A 'Mozilla/5.0' 'https://shade01.bewe.co.kr/api/logs?all=1' | python3 -m json.tool | head -40
> scp 'ku@100.86.239.31:~/shade01-data/logs/*2026-9-11*.ulg' logs/
> ```
>
> 파일명에 **로그 번호가 그대로** 붙어 있어 FC 경로 매핑도 필요 없고, 브리지를
> 세울 일도 없다. 아래 절차는 **아직 웹에 안 올라간** 로그(비행 직후 회수)에만
> 쓴다 — [상세](tools/qgclog/FETCHING.md#-먼저-웹에-이미-올라간-로그면-랩서버에서-받아라-2026-09-11).
...
> 🔴 **파싱 정본은 [`PARSING.md`](PARSING.md) 다.** 읽기 실패를 만나면 고치기 전에
> 그 문서의 §1 진단 순서를 먼저 밟아라 — 파일이 아니라 도구·직렬화·전송이 깨진
> 경우가 훨씬 많았다(여섯 번 중 다섯 번).
>
...
분석 결과를 [shade01.bewe.co.kr](https://shade01.bewe.co.kr) 에서 지도·시간축으로 본다.
...
절차·장애 대응은 [web/README.md](web/README.md).

### 목록은 하위 폴더를 훑지만 `_repair()` 는 아니다

`list_logs()` 는 커밋 `ce43d11` 부터 하위 폴더까지 훑는다 (`ku` 가 실제로
`logs/2026-08-31/` 처럼 날짜 폴더를 쓰고 있었다). **하지만 `_repair()` 는
자기 디렉토리에서만 기증자를 찾는다** ([qgclog.py:469](tools/qgclog/qgclog.py)).

즉 목록에는 뜨지만 **복구는 안 되는** 상태가 될 수 있다. 그래서 위 §1 의
"평면으로 쌓는다" 는 여전히 유효하고, 웹 서버의 업로드 디렉토리도 평면이다.

### FC 경고가 리포트에 안 나오던 문제 (2026-09-03 수정)

`[FC 경고]` 절은 **한 번도 출력된 적이 없었다.** pyulog 의 `log_level` 은 등급
숫자가 아니라 그 숫자의 **ASCII 바이트**여서(`'6'`=54=INFO, `'4'`=52=WARNING),
`log_level <= 4` 가 영원히 거짓이었다.

로그 36개의 FC 메시지 358건 중 **WARNING·ERROR 103건이 통째로 빠져 있었다** —
#184 를 진단한 `Geofence: approaching or outside geofence` 도 이 절에는 안 나왔다.
`_log_level()` 이 정규화한다.

같은 커밋에서 자세 오탐도 고쳤다: 깨진 쿼터니언 1개(norm 4.25e37)가
`log_181` 에 `roll 180° — 제어 상실 의심` 을 만들고 있었다. 고도의 `5.03e14 m` 와
같은 계열이라 유효 플래그로는 안 걸린다. 필터 후 13.1°.
