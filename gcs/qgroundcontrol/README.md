...
### 🐌 백팩 링크는 왜 로딩이 느린가 (2026-09-04 규명)

**대역폭이 근본 한계다. 다만 WiFi 탓이 아니다.** 노트북↔조종기 2.4G WiFi 구간은
빠르다. 병목은 그 **앞단의 ELRS RF 링크**다 — 백팩은 조종기가 이미 받은 텔레메트리를
WiFi 로 중계할 뿐이라, **ELRS 가 상한이면 WiFi 가 아무리 빨라도 그 이상은 못 넘는다.**

ELRS 실측(**333Hz Full** / Telem 1:2 / 13211bps)의 하향 상한은 **1651 B/s** 다
(2026-09-09 조종기 화면 실측). ⚠️ 옛 기록의 `250Hz / 615 B/s` 는 틀렸다.

```
FC ─460800─ RX ─[OTA 약 600 B/s]─ TX백팩 ─WiFi(빠름)─ 노트북 QGC
                      ↑ 병목은 여기다
```

접속 시 QGC 는 파라미터 **1354개**를 받는다. `PARAM_VALUE` 는 메시지당 약 33바이트:

| | |
|---|---|
| 전송량 | 1354 × 33 ≈ **44.7 KB** |
| 이론 최소 시간 | 44700 ÷ 615 ≈ **73초** |
| 실제 | 텔레메트리가 같은 대역을 나눠 쓰므로 **2~5분** |

파라미터가 끝나야 기체설정·미션 화면이 열리므로 "로딩이 오래 걸린다" 로 체감된다.

#### 조치 1 — `noInitialDownloadWhenFlying` (적용됨, rim3)

**비행 중 접속하면 파라미터·미션 다운로드를 통째로 건너뛴다.** 이미 떠 있는 기체에
붙을 때 즉시 화면이 뜬다.

```ini
[Mavlink]
noInitialDownloadWhenFlying=true
```

지상에서 붙을 때는 평소대로 전부 받는다 — **설정 작업에는 영향이 없다.**

#### 조치 2 — 파라미터 캐시를 살려 둔다

QGC 는 기체별 파라미터를 캐시하고, `MAV_HASH_CHK_EN=1` 이라 재접속 시 해시만 대조하고
**전량 재수신을 건너뛴다.** rim3 실측으로 캐시는 정상 동작 중이다:

```bash
ls -la ~/.config/QGroundControl/ParamCache/
# 1_1.v2  (60KB) 가 있으면 살아 있다
```

⚠️ **경로는 `~/.config/QGroundControl/` 이다** — `QGroundControl.org/` 가 아니다.
⚠️ 파라미터를 바꾸면 다음 접속에서 한 번은 전량 다시 받는다. 그건 정상이다.

#### 조치 3 — 대량 전송은 백팩으로 하지 마라

| 작업 | 쓸 경로 |
|---|---|
| 파라미터 조회·수정, 미션 업로드, 펌웨어 | **FC USB 직결**(3번) 또는 **Pi 브리지**(1번) |
| 비행 중 모니터링 | 백팩(2번) |

Pi 브리지는 USB 직결 **21.8 KB/s** — ELRS(1651 B/s 상한, 실제 도착 900~1080 B/s) 대비 **20배**다. 파라미터 로딩이 73초 → 2초.
백팩은 어디까지나 **Pi 가 죽었을 때의 폴백**이다.

#### ❌ `high_latency=true` 로는 고치지 마라

링크 설정의 `high_latency` 를 켜면 미션·지오펜스·랠리 로드를 건너뛰어 빨라 **보이지만**,
QGC 소스상 부작용이 크다:

| 부작용 | 근거 |
|---|---|
| GCS 하트비트 송신 중단 | `MultiVehicleManager.cc:280` |
| `MAV_CMD_CONTROL_HIGH_LATENCY` 전송 — 링크 전환 시 **송신 중단 명령** | `VehicleLinkManager.cc:324` |
| 컴포넌트 정보 요청 생략 | `RequestMetaDataTypeStateMachine.cc:248` |

평상시 텔레메트리가 깨진다. **이 링크는 고지연이 아니라 저대역이다** — 성격이 다르다.
...
### FC 쪽 — `MAV_0_RATE` 를 명시했다 (2026-09-04 변경)

ELRS 링크는 **TELEM1** 로 들어온다:

| 파라미터 | 값 | 뜻 |
|---|---|---|
| `MAV_0_CONFIG` | `101` | TELEM1 에 MAVLink 인스턴스 |
| `SER_TEL1_BAUD` | `460800` | ELRS 수신기 보레이트 — **유선 구간이다. 낮추지 마라** |
| `MAV_0_MODE` | `0` | Normal — 텔레메트리 전량 |
| **`MAV_0_RATE`** | **`490`** | **송신 예산 (B/s). `0`(자동) 에서 변경** |

> 🔴 **`MAV_0_RATE` 는 FC 재부팅해야 적용된다.** `_datarate` 는 mavlink 인스턴스가
> 뜰 때 한 번만 읽는다 (`mavlink_main.cpp:2203`). 값만 쓰고 재부팅을 안 하면
> 아무것도 안 바뀐다 — 실측으로 확인했다.

> ⚠️ `MAV_0_RATE` 는 **TELEM1 인스턴스에만** 적용된다. USB 링크는 별도 인스턴스라
> 영향이 없다 — **USB 로 QGC 를 보면서는 이 값의 효과를 검증할 수 없다.**

> ⛔ **아래 「왜 바꿨나」는 오진이었다 (2026-09-09 정정). 현재 값은 `1400` 이다.**
>
> 250Hz 전제가 틀렸고(실제 **333Hz Full**, 상한 **1651 B/s**), 진짜 원인은 대역폭이
> 아니라 **`MAV_0_FORWARD=1`** 이었다 — USB 인스턴스로 들어온 트래픽이 TELEM1 로
> 넘어가 송신량의 **95%** 를 먹고 있었다. `MAV_0_RATE` 를 490·990 으로 옮겨도 증상이
>그대로였던 이유가 이것이다.
> [경위와 실측](../../FC_CHANGELOG.md#-2026-09-09--sensor-lost-원인-규명과-해결-mav_0_forward--extrastxt--mav_0_rate)

~~**왜 바꿨나.** ELRS 실측 설정이 **250Hz / Telem 1:2 / 4921bps** 다. 하향 텔레메트리
상한이 **615 B/s** 인데, PX4 는 9/2 비행에서 **593 B/s** 를 내보내고 있었다 —
**상한의 96%, 여유 3.6%**. 조금만 튀어도 RX 의 1024바이트 버퍼가 넘친다.~~

~~`490` 은 상한 615 의 **80%** 로, 나머지를 UART 버스트 여유로 남긴 값이다.~~
근거·증상은 [ELRS 링크 대역폭 포화](../../components/receivers/radiomaster-rp4td-m/README.md) 참조.
