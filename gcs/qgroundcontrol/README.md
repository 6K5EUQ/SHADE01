...
**대역폭이 근본 한계다.** 노트북↔조종기 WiFi 구간은 빠르지만, 그 앞단인
**ELRS OTA 가 실측 583~638 B/s** 밖에 안 나온다. 백팩 경로도 결국 TELEM1 을 지난다.
...
Pi 브리지는 USB 직결 **21.8 KB/s** — ELRS 대비 **36배**다. 파라미터 로딩이 74초 → 2초.
...
| **`MAV_0_RATE`** | **`700`** | **송신 예산 (B/s). `0`(자동) 에서 변경** |
...
**왜 바꿨나.** `MAV_0_RATE=0` 이면 PX4 가 예산을 `보레이트/20 = 23040 B/s` 로 잡는다
(`mavlink_main.cpp:2205`). 그런데 그건 **유선** 보레이트다. 실제 ELRS 무선 구간은
**583~638 B/s** — 약 **38배** 차이다. PX4 는 없는 대역폭을 믿고 58개 스트림을 밀어넣고,
ELRS RX 의 1024바이트 버퍼가 즉시 넘친다.

`700` 은 실측 평균 약 600 B/s + 헤드룸 15% 다.
