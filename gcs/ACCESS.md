...
| `rim` | 100.107.83.47 | **`rim`** | `~/SHADE01` | 호스트명은 `RIM`. RTT ~0.3ms. 🟡 **수동 갱신 전용** (2026-09-10 결정) — 원격 조작 안 함, 그 PC 에서 `git pull` |
| ~~`raspb1-dgs3`~~ | ~~100.126.161.1~~ | — | — | 🔴 **쇼트 사망** (2026-09-10 확정). 기체 컴패니언이었다 — 기록은 [컴패니언 문서](../components/companion/raspberry-pi-5/README.md) |
...
🟡 **`rim` 은 풀지 않기로 했다** (2026-09-10) — fetch 만 되는 상태를 그대로 두고, **그 PC 에
앉아 `git pull`** 로만 갱신한다. 원격 push·scp·ssh 자동화의 대상이 아니다.
