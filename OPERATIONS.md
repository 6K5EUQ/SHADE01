...
기체 식별 · 링크 구성 · 현재 상태 · 다음 비행 전.
소개는 [`README.md`](README.md), FC 변경 이력은 [`FC_CHANGELOG.md`](FC_CHANGELOG.md) 다.
...
잡고, **백팩 경로 실측 송신은 936 B/s** (예산의 67%, 상한의 57%) 다 — 2026-09-11
90초 측정, `BAD_DATA` **0**. `txbuf` 100, `rate_multiplier` 1.000 — 지금은 깎이지 않는다.

**설정한 Hz 는 94% 가 실제 도착한다** (자세 10Hz → **9.4Hz** 실측, 2회 재현).
재는 법은 [tools/fc/MEASURING.md](tools/fc/MEASURING.md) —
🔴 **USB 로 본 Hz 는 TELEM1 의 Hz 가 아니다** (스트림 레이트는 MAVLink 인스턴스별).
...
| 🟡 기압계 | GPS 대비 **−10.4m** (2026-09-11 실비행 10편, 공중 구간 중앙값. 범위 −12.4~−7.6, 편차 1.6m). 한 비행 안에서는 ±1m 로 안정하고 **날마다·시간대마다 달라진다** — 특정 값으로 보정할 대상이 아니다. 미션·RTL(홈 기준 상대고도)엔 무관, **GPS 를 잃는 순간** 이 오차가 고도에 실린다 |
| ✅ `NAV_DLL_ACT` | `2`(RTL) — 2026-09-05 14:16. 데이터링크 두절 시 복귀 |
| 🟡 수동 천이 개방 | **스위치 천이만 열었다** (2026-09-17). `RC_MAP_TRANS_SW` 0→**7** (SD/CH7) · `VT_ARSP_TRANS` 10→**14** m/s (영점 미판정 흡수) · `VT_ELEV_MC_LOCK=1` 유지 · **미션 이착륙 `22`/`21` 유지 → 자동 천이는 계속 차단** |
...
## 🟡 천이 — 수동만 열림 (2026-09-17 갱신)

> **2026-09-04 ~ 2026-09-17: 쿼드 전용으로 완전 차단** 이었다.
> 2026-09-17 에 조종자 지시로 **SD 스위치 천이만** 열었다.
> 자동 천이(미션 `84`/`85`)는 **여전히 막혀 있다.**
> 아래 절은 그때의 차단 절차 기록이며, 지금 상태는
> [`FC_CHANGELOG.md`](FC_CHANGELOG.md) 2026-09-17 절이 정본이다.

### 원래의 차단 (2026-09-04)

**이 기체는 당분간 멀티콥터(쿼드) 모드로만 운용한다.** 고정익 천이는 하지 않는다.

> 🔴 **유지한다** (2026-09-11 조종자 지시, 2026-09-13 근거 갱신).
>
> 1. **배율이 미보정이다.** `ASPD_SCALE_1=1.0` 인데 9/11 실비행 로그에서 잰
>    기울기가 **1.15** 다 (log_339, 기수방향 속도 대조). 특히 **고속 구간에서
>    1.43배 과대평가** — 실속 판단(`FW_AIRSPD_STALL=7`·`FW_AIRSPD_MIN=10`)이
>    걸리는 속도대가 바로 거기다. 실제 7 m/s 를 10 으로 읽으면 경보를 놓친다.
> 2. **영점은 개선됐지만 잔차가 남는다.** 9/11 실비행 정지 구간(GPS<0.5m/s)에서
>    세션 초 −3.8 → 세션 말 **−2.9 m/s**. 보정이 들어간 것이 데이터에 보인다.
>    센서 자체는 살아 있다 — 움직일 때 GPS 와 **상관 0.77~0.79**.
>
> ⚠️ 전에 여기 있던 「저녁 실측 +2.21 m/s = 과보정」은 **철회했다** — 실내에서 잰
> 값이고 에어컨 바람이었다 ([경위](FC_CHANGELOG.md#-2026-09-11-저녁--에어스피드-재보정-sens_dpres_off--18096---30643)).
> 값은 실비행 로그에서 뽑는다 — [원칙](CLAUDE.md#-값을-뽑을-때--실내-조회값과-실비행값을-섞지-마라).
>
> 푼다면 **영점을 −1.8096 쪽으로 되돌려 정지 ±2 를 다시 확인한 뒤**, 높은 고도 ·
> 큰 여유 · 원선회로 첫 비행을 잡아라 (「다음 비행 전」 2·3번).
> 참조: [영점 검증 경위](FC_CHANGELOG.md#-2026-09-11--에어스피드-영점-검증-sens_dpres_off--452---18096)
...
**푸는 순서** — ✅ 1단계(에어스피드 영점)는 2026-09-11 에 끝났다. 남은 것은
`RC_MAP_TRANS_SW=7` 로 되돌리는 것이고, 그 첫 비행이 곧 **스케일 학습 비행**이다
(「다음 비행 전」 3번의 원선회 조건). `FW_AIRSPD_STALL=7.0` 은 **공장 기본값이라
이 기체의 실측 실속속도가 아니다** — 스케일이 잡히기 전까지 그 숫자를 믿지 마라.
...
2. 🟡 **에어스피드 — 영점은 개선, 배율이 남았다** (2026-09-11 실비행 10편 실측).

   정지 구간(GPS<0.5 m/s)의 영점이 세션 중 **−3.8 → −2.9 m/s** 로 개선됐다.
   움직일 때 GPS 와 **상관 0.77~0.79** 로 센서는 정상이다 — 예전 「−5 m/s 고정」
   고장 상태와 완전히 다르다.

   남은 것은 **배율**이다. log_339 를 기수방향 속도로 대조하면:

   | 구간 | 에어스피드 | 기수방향 속도 | 비 |
   |---|---|---|---|
   | 2~3 m/s | 0.66 | 2.00 | 0.33 |
   | 3~5 m/s | 3.95 | 4.15 | **0.95** ✅ |
   | 5~8 m/s | 8.58 | 5.99 | **1.43** ⚠️ |

   중속은 맞는데 **고속에서 1.43배 과대평가**다. 실속 판단이 걸리는 구간이라
   이대로 고정익을 쓰면 위험하다.

   ⚠️ **실내에서 재지 마라.** 정지 상태인데 +2.2 m/s 가 나오는데 에어컨 바람이다.
   2026-09-11 저녁에 그 값으로 「과보정」이라 판정했다가 철회했다 —
   [원칙](CLAUDE.md#-값을-뽑을-때--실내-조회값과-실비행값을-섞지-마라).

3. **에어스피드 스케일 학습** — `ASPD_SCALE_1` 이 **1.0**(미보정)이다. 영점(압력 0점)과 달리
   **비례 오차라 지상에서 못 잡는다.** 조건은 이미 갖춰져 있다 (`ASPD_SCALE_APPLY=2` 비행 중 적용,
   `ASPD_SCALE_1=1.0` 이라 첫 5분 학습 가속). 단 **고정익 구간이 있어야** 하고
   (MC 비행으로는 안 잡힌다), 검증된 스케일은 **12개 방위 구간(30°씩)** 을 채워야 갱신되므로
   **첫 고정익 비행에 원선회를 넣어라.** 참조 기체는 1.0742 를 학습했다.
4. **TELEM1 스트림 비행 중 확인** (2026-09-11 변경분). 지상 실측은 끝났고 **비행 중이 미검증**이다.
   - `BATTERY_STATUS` **5Hz** — 지상에서는 배터리 미연결이라 못 쟀다. 전류 첨두(9/5 의 81.2A 는 **0.20초**)를 잡는지.
   - `rate_multiplier` 가 **1.000** 을 유지하는가. `MAV_0_RADIO_CTL=1` 이라 거리가 멀어지면 PX4 가 깎는다.
   - `SYS_STATUS` 최대공백이 커지는가 (지상 2차 실측에서 **6.06초**. EdgeTX 임계 20초).
   - 재는 법: [tools/fc/MEASURING.md](tools/fc/MEASURING.md) · 되돌릴 백업은 `rim3:~/fc-backup/`
5. **홈 위치 확인** — arm 후 QGC 지도에서 홈(H) 이 기체 위인지. 펜스·RTL 이 전부 홈 기준이다.
6. **RX failsafe 실측** — 조종기를 끄고 FC 가 RC 상실을 몇 초 만에 인지하는지 본다.
   ELRS RX 가 "마지막 값 유지" 로 설정돼 있으면 `NAV_RCL_ACT=2` 가 늦게 걸리거나 안 걸린다.
7. **기압계 −10.4m 오차** — GPS 대비 (9/11 실비행 10편 실측). `EKF2_HGT_REF=1` 이라
   GPS 가 있는 동안은 무해하지만, **GPS 를 잃으면 그 순간 10m 가 고도에 실린다.**
   ⚠️ 실내에서 재면 −38m 까지 나온다 — GPS 수직오차 때문이지 기압계 탓이 아니다 (아래 「값을 뽑을 때」).
8. **자기 간섭 저감** — EKF `mag_field` **3축 전부** 이상. GPS 마스트 높이기, 전력선 이격·트위스트.
...
| 날짜 | 사건 |
|---|---|
| 2026-09-11 | ✈️ **야외 10편 20분 22초.** 미션은 **0회** — CH6 가 988(슬롯1)에 한 번도 안 갔다(기체 문제 아님). 최대 **188m·59.3m**, `rc_lost` 0. 🔴 나침반 **−0.95** 역대 최악, 배터리 누적 12916 mAh, 최대 81.8A. [RTL 4회 실측](flights/2026-09-11-rtl-measured.md) · [세션 전체](flights/2026-09-11-outdoor-session.md) |
| 2026-09-11 | ✅ **TELEM1 스트림을 보조자 화면 기준으로 재배분.** 비행은 항상 2인 — 조종자는 육안 수동조종, **보조자가 실시간 화면을 본다.** 그래서 예산의 수요자는 조종기가 아니라 웹 뷰어다. 자세 `5`→**`10Hz`**(실측 9.4), EKF·진동 되살림, 배터리 `4`→**`5`**. 실측 **936 B/s = 예산 67%**, `BAD_DATA` 0, 여유 464 B/s. 🔴 **「설정 Hz 의 1/3만 온다」 는 추정은 틀렸다 — 94% 온다** (그 근거였던 9/6 의 1.6Hz 는 `MAV_0_FORWARD` 고장 시절 값) ([상세](FC_CHANGELOG.md#-2026-09-11-1032--남은-예산을-보조자-화면에-ekf진동-되살림)) |
...
| 2026-09-11 | ✅ **라이브 화면 기본 수신 경로를 ELRS 로 뒤집었다** (`LINK_PRIORITY`). USB 는 지상에서만 붙어 있는 경로라, USB 우선이면 이륙 전과 비행 중이 **서로 다른 링크**로 보였다 — 백팩 문제를 땅에서 못 잡는다. 실측으로 대역폭 걱정도 접혔다: 백팩 도달률 **94%**, `BAD_DATA` **0**, 자세 **9.4Hz**. 배지 고정 순서도 자동 → ELRS → USB 로 따라 뒤집었다 |
